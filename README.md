...
> 当前状态：产品 Core 已统一为三项——桌面空白处右键创建并管理盒子、盒子绑定真实文件夹、任务栏美化。BOX-R1-A/B、FOLDER-R1-A～D 与 PF-007A/B 工程链已完成，真实可见交互证据仍 Pending；TASKBAR-R1A～R2B1-A2 已建立只读、恢复、原生边界、可丢弃环境准入和两张有限预设卡片。Stage 240 已关闭 Live UI Runtime 预检的宿主与准入 fail-open：入口不再解析 PATH 中的裸 `powershell`，而是在当前受信进程中执行精确仓库脚本，并且只有 schema 5 的完整 `Pass` 或显式确认的已知上游风险结果能够进入应用/UIA 启动。工程基线为 1,394/1,394，但 M1/M2 顶层产品旅程仍为 `0/2 Complete`。当前电脑可枚举 Framework 2.4.0、Singleton 8002.4.0.0，仍缺少 Main.2 `>=2.3.1.0` 与项目锁定 DDLM `2.3.1.0-x6`，现有 XAML 组合也仍属于已知风险；任务栏、许可证、Publisher、托管签名和安装生命周期外部门禁仍未满足。详见 [PRODUCT_EXECUTION_PLAN](docs/PRODUCT_EXECUTION_PLAN.md) 与 [Stage 240](docs/240-live-ui-runtime-preflight-host-integrity-audit.md)。所有产物仍不可公开分发。
...
- [当前 Live UI Runtime 预检宿主与准入完整性审计（Stage 240）](docs/240-live-ui-runtime-preflight-host-integrity-audit.md)
