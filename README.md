...
> 当前状态：已进入 Phase 0。桌面/Shell 数据链、图像资源、DesktopHost 原生窗口模型和静态双屏混合 DPI 子探针已经完成；交互/无障碍与显示器热切换矩阵尚未关闭。正式 MVP 尚未开始。
...
继续 Phase 0 双轨验证：交互轨建立首次整理、拖放语义和撤销原型；技术轨在受控实验室验证 Win+D、全屏、无障碍和显示器热切换，并补充 `QueryDisplayConfig` 与缩略图工作进程恢复。只有体验与技术风险同时通过，再进入 MVP 开发。
