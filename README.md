...
按[统一开发计划](docs/PRODUCT_EXECUTION_PLAN.md)执行 TASKBAR-R2A2：在独立 Worker 中接入受认证的系统默认/清透原生适配器与恢复执行器。Build 26200 保持失败关闭；M1 物理证据在安全运行时和专用环境具备后恢复；自动整理、Tab、小组件、工作空间和 Long助手运行时不得抢占三项 Core。
