...
> 当前状态：PF-010B1 已把删除、布局、文件夹绑定/解绑、引用增删/改归属/排序和布局恢复接入统一 50 步历史，批量动作保持一步语义，真实目录、Unicode 文件 SHA-256 和保存失败历史补偿通过，完整 Release `1,447/1,447`，状态为 `EngineeringComplete / RealFilesystemPass / ProductEvidencePending`。PF-010B 整体仍为 `InProgress`，唯一接续点是 PF-010B2 重启后最近一次安全恢复点；M1/M2 仍为 `0/2 Complete`、30 项 PF 仍为 `0 Complete`，所有产物不可公开分发。完整审计见 [Stage 266](docs/266-pf010b1-unified-history-action-breadth-audit.md)，统一执行源见 [PRODUCT_EXECUTION_PLAN](docs/PRODUCT_EXECUTION_PLAN.md)。

## 产品原则

1. **功能优先**：先交付用户能看到、能操作、能验证的核心能力和完整旅程，再扩展工程辅助设施。
2. **零惊吓**：绝不擅自移动、删除或加密用户文件；所有自动化均可预览、撤销和恢复。
3. **轻量常驻**：启动快、空闲占用低，不干扰 Explorer、游戏、演示和多显示器切换。
4. **本地优先**：核心能力无需登录和联网；用户数据默认只留在本机。
5. **融入 Windows**：遵循系统交互、辅助功能、DPI、主题和快捷键约定。
6. **安全不退化**：权限、安全、恢复和发布门禁是功能验收底线，不作为脱离用户结果的连续开发主线。

## 文档导航

当前工作只需要优先阅读以下文档：

- [PF-008C 自定义引用顺序与恢复审计（Stage 262）](docs/262-pf008c-custom-reference-order-audit.md)
- [PF-009A 共用搜索模型与正式结果列表审计（Stage 263）](docs/263-pf009a-shared-search-results-audit.md)
