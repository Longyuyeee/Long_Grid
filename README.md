...
> 当前状态：处于 Phase 0 退出收尾。正式配置工作区、只读桌面 Catalog、连续保存/恢复和开发期内部 RC 构建链已经进入主线；E1/M1 正在把正式 DesktopHost 补齐为默认关闭、可补偿的 Passive/Explicit/Hidden 表面，但正式输入源与 Intent 消费仍未接入。桌面文件内容读取、写入和移动继续保持关闭。#19、#20、#23、#24 的人工/硬件/真实卷证据、ADR-0001 决策、许可证、签名与安装生命周期仍未完成，因此所有产物均不可公开分发。当前唯一收尾顺序和逐项验收门槛见 [Phase 0、桌面 MVP 与内部 RC 收尾执行计划](docs/125-phase0-internal-rc-closeout-plan.md)，E1 专项边界见 [Stage 130](docs/130-formal-explicit-surface-adapter-audit.md)。
