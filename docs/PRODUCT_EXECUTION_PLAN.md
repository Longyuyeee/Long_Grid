...
| A1 首次创建 | 在隔离配置下实际启动 A0 EXE，进入控制中心创建空盒子；在无 SDK/无预装项目运行时的干净 Windows 11 x64 测试环境复测 | 无脚本/环境变量/人工安装依赖，窗口正常显示；点击创建后桌面真的出现唯一盒子；保存后正常退出，以同一配置重启仍在 | 未通过 |
...
| A4 正式安装入口 | 获准的发布身份/签名和可丢弃环境到位后安装 MSIX；测试开始菜单、桌面右键创建、单实例及 Explorer 重启 | 用户正常安装即可启动和右键创建；不要求导入开发证书、关闭安全策略或运行 PowerShell | 未通过；有外部前置条件 |
