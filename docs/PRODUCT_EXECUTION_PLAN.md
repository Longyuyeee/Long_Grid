...
| 文件夹绑定 | 有安全引用、目录枚举、打开与 Portal/Tab 设计底座 | FOLDER-R1-A～D 已完成有界内容与有限状态；#267～#270 已补齐可见路径、三种持久化基础排序、真实加载状态和一次性 path-free 恢复来源 | 工程链与真实文件系统已通过；恢复不覆盖真实内容状态、不持久化；物理 Picker、可见状态/刷新/打开/排序、键盘和 Narrator 仍 `ProductEvidencePending` |
...
FOLDER-R1 当前绑定路径可见性纠偏记录（2026-08-27，基线 `origin/main@d069f11`）：

| 检查 | 预期效果 | 实际效果 | 差异与修正 |
|---|---|---|---|
| 原始需求对齐 | PRD 要求显示当前绑定路径，且用户应能在所选盒子上下文确认绑定目标 | 审计发现正式管理页只有 `Resolved / Missing / AccessDenied / Replaced / Unavailable` 状态文字，没有路径控件 | 确认是产品可见结果缺失，不把它误写为纯证据 Pending；新增默认折叠、选中绑定盒子后显示且可复制的“当前绑定路径” |
| 选择与陈旧状态 | 路径只能属于当前编辑修订和当前所选盒子；刷新或编辑后不能继续显示旧路径 | 新 Core 策略要求 requested/current edit revision 精确相等、序号在界内且盒子当前仍有绑定；陈旧修订、非法序号、未绑定和控制字符路径全部返回无路径 | 第一版把路径放入容器编辑 presentation，触发既有“presentation 不得携带持久化身份”合同；已撤回并改为 App 组合根中的有限即时解析委托，候选列表继续无身份 |
| 隐私与无障碍 | 本地用户和 Narrator 能读取明确选择的路径，但概览、DesktopHost、遥测和机器状态不得携带路径 | 路径只进入独立 `TextBlock` 的可见文字/无障碍名称；通用 read model、容器 editor presentation 与 `AutomationProperties.ItemStatus` 保持无路径 | 对齐；这是用户明确管理上下文中的本地显示，不改变“不上传路径、默认诊断脱敏”的安全基线 |
| 回归门禁 | 纯策略、Release、全量测试、覆盖率、格式和 UI 合同继续通过 | 路径策略专项 9/9；全量 1,362/1,362；Release 0 warning / 0 error；lines 90.41%、branches 76.06%；196-ID UI ContractOnly 通过 | `Difference=None`；功能不读取目录内容、不保存配置、不执行文件或系统操作 |
| 可见物理证据 | 在真实绑定盒子中观察长路径、Unicode、键盘复制和 Narrator 朗读 | 当前跨进程 WinUI/UIA 风险与可丢弃环境门禁未变化，本轮没有绕过风险启动外部自动化 | `ProductEvidencePending`；M1 完整物理旅程出口不降低 |
...
FOLDER-R1 基础内容排序需求纠偏记录（2026-08-27，基线 `origin/main@ffdff3a`）：

| 检查 | 预期效果 | 实际效果 | 差异与修正 |
|---|---|---|---|
| 原始需求对齐 | PRD 第 4.1 节要求绑定目录内容支持排序；FOLDER-R1 完成口径至少要有用户可选择且重启稳定的基础顺序 | 代码只有 `WindowsProductContainerFolderContentReader` 内部固定“文件夹优先、名称升序”，控制中心无内容排序入口，schema v3 无排序字段 | 确认为 P0 遗漏而非物理证据债；补“文件夹优先名称升序 / 名称升序 / 名称降序”，不扩张到 PF-008 的视图、间距、时间或自定义拖序 |
| 持久化与迁移 | 旧配置保持原有视觉顺序，新选择按盒子保存，未来未知 schema 继续失败关闭 | workspace binding 与配置 binding 新增有限枚举，schema 升至 v4；v3 强制迁移为旧默认，v1/v2 原有安全迁移保持，v5 继续拒绝 | 对齐；排序偏好随绑定身份投影和恢复，目录项目、路径清单与文件属性不进入配置 |
| 原子提交与撤销 | 排序不能绕过锁定、陈旧 revision、只读和保存失败边界；可复用最近一次编辑撤销 | 保存入口以当前 edit revision/所选序号取得当前绑定，只提交 binding 的 `SortMode`；现有 `BindFolder` reducer/CAS/保存补偿/`FolderBinding` undo 全部复用；重新连接文件夹保留当前排序 | 对齐；无新旁路事务，未改变真实文件或绑定 NTFS 身份 |
| 内容投影与文件安全 | 三种模式产生稳定 UIA/视觉顺序；枚举仍有界、直属、跳过重解析点且零写入 | 真实临时目录中的文件夹与文件分别验证三种顺序；仍最多检查 1,024 项、投影 256 项，排序后生成同代有限 Item ID；前后文件哈希和路径不变 | `Difference=None`；非法枚举在读取前失败关闭为 `EnumerationFailed` |
| 用户入口与无障碍 | 只有已绑定、未锁定且可编辑的所选盒子可保存；选择与保存入口有明确 UIA 名称和有限反馈 | “内容排序”选择器与“保存排序”按钮接入基本设置；未绑定/锁定/只读默认禁用，状态播报含模式、revision、Changed 和 `DesktopFilesChanged=False` | 198-ID UI ContractOnly 已通过；物理键盘、Narrator 和可见刷新仍进入 M1 |
| 回归门禁 | 格式、Release、专项、全量、覆盖率和 UI 合同不得退化 | Release `0 warning / 0 error`；相关配置/真实目录/提交专项 `80/80`；全量 `1,366/1,366`；coverage lines `90.40% (23477/25969)`、branches `76.05% (7683/10103)`；格式与 198-ID UI ContractOnly、M1 ValidateOnly、文件安全和依赖漏洞门禁通过 | `Difference=None`；未把源码合同写成物理产品证据 |
...
PR #268 Windows runner run `33090008325` 完整通过：`1,366/1,366`，coverage lines `90.09% (46790/51938)`、branches `75.93% (15342/20206)`；198-ID UI/批量无障碍/clean-session 合同、格式、Release 构建、启动与恢复、真实文件安全、原生宿主、受限缩略图 Worker、依赖漏洞和内部未签名 RC 交付集均成功。证据回填后的 run `33091209405` 再次全绿；PR 无评论并 squash 合入 `main@14e1cae`，远端与本地功能结论无差异。

FOLDER-R1 加载状态可达性纠偏记录（2026-08-28，基线 `origin/main@14e1cae`）：

| 检查 | 预期效果 | 实际效果 | 差异与修正 |
|---|---|---|---|
| 原始需求与可达性 | PRD 要求显示加载状态；枚举存在不能替代真实生产状态可达 | `AwaitingRefresh` 只出现在 Core 枚举、health 排除条件和 UI 默认文案，没有任何构造调用；首次绑定/刷新将 `folderContents` 置空，read model 实际输出 `Content=None` | 确认为 P0 状态缺口；新增 path-free `CreatePending`，在读取任务启动前按当前 generation 发布 |
| 状态有限性 | 已解析绑定进入加载；权限拒绝、无效和失效绑定不应伪装加载 | Resolved 映射 `AwaitingRefresh`；AccessDenied/InvalidTarget 映射同名内容状态；Missing/Replaced/Unavailable 映射 `BindingUnavailable` | 对齐；pending 只含盒子 ID、generation、有限状态与绑定 resolution，不含路径或项目 |
| 并发与最终一致 | 旧刷新不能覆盖新工作区或新 generation，完成前也不能把旧内容包装成当前结果 | 每次接受刷新先递增 generation、替换为 pending，再启动有界读取；loading UI 投影复核 generation 和当前仍为 `AwaitingRefresh`，完成发布继续复用 state 引用/revision/generation 三重校验 | 对齐；快速连续刷新仍由取消令牌与发布校验失败关闭 |
| 健康与用户反馈 | 加载不是错误，不能把盒子标成待审查；控制中心明确说明正在加载 | read model 保持 `Health=Ready`、0 个临时项目；正式状态文字显式映射“正在加载绑定文件夹内容”，UIA ItemStatus 输出 `Content=AwaitingRefresh` | 对齐；加载完成后同一正式投影切换为 Ready/Empty/有限失败 |
| 形状与回归 | 未定义状态和非法计数必须拒绝；格式、Release、专项、全量、覆盖率与 UI 合同不得退化 | 新形状校验拒绝未定义 status 与负 skipped count；相关 Core/read model/真实目录专项 `26/26`；全量 `1,374/1,374`；coverage lines `90.42% (23505/25996)`、branches `76.08% (7699/10119)`；Release `0 warning / 0 error`，198-ID UI ContractOnly 通过 | 当前工程差异为 `None`；物理可见加载时序和 Narrator 仍 Pending |
...
PF-007A2 的完成边界：正式产品已经拥有原生 OLE DropTarget 和端到端安全引用提交工程链，真实注册、有限效果、一次提交及文件零变化已证明；尚未取得物理 Explorer 指针和屏幕可见证据，因此不能标为用户旅程 Complete。A2 邻接底座不再扩展，其物理证据并入 M1。

PF-007B DesktopHost 盒子间改归属记录（2026-08-26，基线 `origin/main@2980857`）：

| 检查 | 预期效果 | 实际效果 | 差异与修正 |
|---|---|---|---|
| 手势与反馈 | 从已选正式引用拖到另一盒子，短点击不误提交也不吞掉原动作，安全目标有反馈 | 首轮阈值内释放零提交但吞掉单击；已修正为恢复原选择/单击打开链，真实 HWND 实际各调用一次；6 DIP 后才拖动，同显示器安全目标绘制焦点框 | 首轮差异已修正；最终 `Difference=None` |
...
专项真实探针预期进程存活、四阶段齐全、配置隔离、正常配置零变化和夹具零变化，实际全部通过，`Difference=None`。CI 只运行 `-ValidateOnly` 验证入口合同，不把无交互 runner 当作物理证据；M1 在环境可用后恢复。环境阻塞期间已受控完成 TASKBAR-R1A/R1B，详见 [Stage 210](210-taskbar-r1a-readonly-compatibility-audit.md)与 [Stage 211](211-taskbar-r1b-bounded-client-personalization-audit.md)。
...
| 签名与生命周期否定性合同 | 环境存在不能把签名、安装和分发状态自动升级为可用 | 环境准入时 `Test-LongGridReleaseSigning.ps1 -ValidateOnly` 仍为历史状态 `BlockedPendingApprovedPublisherCertificateAndEnvironment`，PR/main signing access、live signing、install/distribution 均为 false；MSIX lifecycle 的 `liveEvidence` 仍为 `PendingSignedPackageAndDisposableWindowsProfile`，不改包状态、不信任 unsigned 包 | 首次组合包装器错误把 lifecycle 顶层 `outcome=Pass` 当成物理状态，实际 Pending 位于 `liveEvidence`；修正为同时断言合同 Pass 与 live evidence Pending 后复测 `Difference=None`。环境真实建立后，当前状态值在 13.6 进一步纠正 |
...
真实安全复测 Expected 为“增加责任入口不改变任何发布权限”；Actual 为 `long-grid-release` secrets `0`、variables `0`、deployments `0`，当时 `Test-LongGridReleaseSigning.ps1 -ValidateOnly` 仍返回历史状态 `BlockedPendingApprovedPublisherCertificateAndEnvironment`、`liveSigningImplemented=false`，Difference=`None`。开发目标审计：发布外部输入已有唯一负责人入口且许可证依赖没有被覆盖，已完成；负责人批准和外部输入本身仍未完成。需求对齐审计：Issue 只建立责任、禁止项和验收，不替负责人选择许可证/Publisher，不生成或接触秘密，也不创建签名 workflow。下一唯一接续点是负责人在 #274 提供 Publisher/托管签名方案并在 #23 处理 D23-11；在此之前工程状态保持阻断，不扩张相邻发布机械链。
...
### 13.22 Stage 244：M1 证据模式分离与 Runtime 准入防误判
...
### 13.23 Stage 245：M1 cleanup 模式隔离与证据生命周期防遮蔽

从 `main@2f93ec8` 复读外部条件，#23/#274、受保护签名、许可证与本机 Runtime 均未变化，BOX-R1-C/D 继续停止。真实创建精确 marker 会话后发现 `-ExternalAutomation -CleanupSessionId` 先被 Runtime 预检阻断并留下目录；修正为在 Runtime 和证据操作前拒绝 Cleanup 与 ValidateOnly/ExternalAutomation 的组合，调用方必须分开执行。真实非法组合 exit 1、目录保持、LongGrid 进程不变；合法 cleanup 删除同一目录。新增真实 Windows PowerShell 子进程回归，完整本机测试 `1,397/1,397`，coverage `90.46%/76.16%`，漏洞 0，许可证继续禁止分发。首次 coverage 因 43 份历史结果被聚合而正确失败，精确清理未跟踪 TestResults、关闭 build server 并生成唯一结果后通过；未降低阈值。PR #320 与精确合并提交 `main@19c929d` 的 CI/CodeQL 均成功；main 测试 `1,397/1,397`、coverage `90.14%/76.04%`、双语言 open alerts=0。产品状态和下一唯一接续点不变，详见 [Stage 245](245-m1-cleanup-mode-isolation-audit.md)。

### 13.24 Stage 246：M1 可见窗口 Ready 与自包含 Runtime 真实审计
...
### 13.32 Stage 254：统一执行计划当前队列新鲜度合同
...
### 13.34 Stage 256：M1 marker 前空会话目录清理
...
### 13.36 Stage 258：M1 正式 App 精确 marker 消费安全
...
### 13.44 Stage 266：PF-010B1 统一历史动作广度

删除、布局预设/拖动、文件夹绑定/解绑、引用单个/批量增删、批量改归属、自定义顺序和布局恢复进入同一 50 步历史；批量动作保持一步，Failed-save 兼容补偿同步撤回假成功历史。初始布局红测 Expected 为“调整方格布局”、Actual 为空集合；修正后专项 `20/20`、相关 `33/33`、完整 Release `1,447/1,447`。真实中文目录/文件旅程与 lease 失败补偿保持 SHA-256 和 sentinel 不变；Release 0 warning/error、211-ID UI 合同通过。PF-010B1 工程收口但 PF-010B 整体仍 InProgress；唯一接续点为 PF-010B2 重启后最近一次安全恢复点，详见 [Stage 266](266-pf010b1-unified-history-action-breadth-audit.md)。
