...
2026-09-23 当前版本取消时序修正：实际 DesktopWorkspaceCreatePreviewWindow.ShowAsync 在 Activate 后等待 150ms，再无条件操作 AppWindow；期间取消会 Complete(null) 并 Destroy 窗口，延迟恢复后可能访问已销毁窗口并触发上层异常回退。现通过 UI dispatcher 上的完成状态检查跳过已结束预览的呈现；内部 evidence 路径的两个 await 后也检查完成状态，避免取消后再设置名称/确认。新增链接真实 App continuation 源码的 3 项回归覆盖取消、已确认结果保留和仍在等待的正常呈现；全量 1,531/1,531（17 秒、零跳过）、App Release 零警告/错误。该修正来自代码时序审计和单元验证，不声称已在真实 WinUI 窗口复现/点击通过；A1 增加“预览刚出现即取消/切走”的物理回归，必须零创建且不出现二次确认。现有包不包含本次改动，后续候选从新提交重建。

#### 当前版本收口与后续版本规划（2026-09-23）

用户要求当前版本收尾发布后继续规划提升。当前版本仍以原 PRD 三项核心为范围，不以新增功能替代修复，不因“要求发布”忽略未通过项。先关闭 A1 创建/取消/保存/重启与崩溃，再按 A2～A5 验证桌面基本操作、文件夹引用、安装生命周期及任务栏核心，最后 A6 签名/许可证/兼容性/稳定性/可追溯产物。版本号及日期在候选范围与验收结果确定后指定；0.1.0.2 只是内部未签名包版本，不能当正式发布。

下一版本候选按用户影响排序，当前只规划、不提前开发：

| 优先级 | 用户问题/候选内容 | 决定进入版本的依据 |
|---|---|---|
| P0 热修复 | 创建失败、闪退、保存丢失、升级后菜单失效 | 任一可复现报告即优先处理；必要时单独补丁，不等待功能版本 |
| P1 易用性 | 创建失败时就地解释与重试、异常默认名回退体验、清楚的保存状态 | 当前 A1 实测及首次使用反馈；不得依赖控制中心才能恢复 |
| P1 桌面适配 | 多屏/高 DPI、显示器断连后找回盒子、键盘及输入法操作 | 真实设备矩阵缺陷与可复现步骤；基础兼容阻断应在当前版解决 |
| P2 文件夹体验 | 大目录滚动、刷新/监控稳定性、失效路径恢复、缩略图回退 | 在原文件不变前提下取得规模和响应时间数据 |
| P2 后续增强 | 搜索、布局恢复、整理规则等已有工程功能的用户体验收口 | 三项核心稳定且有真实需求；不默认扩展真实文件移动、插件或小组件 |

正式发布后复盘同一版本的已知问题、实际用户反馈与回归证据，逐项写明影响、复现、修复版本和验收方式；不以测试数量替代使用效果。不新增遥测上传或自动收集用户桌面资料。未实际发布前不宣称已收集发布反馈，用户今后是否提供反馈由用户决定；本计划不是定时后台监控任务。
...
用户已明确：主路径是 **Windows 桌面空白处右键 → 新建 Long方格盒子 → 桌面直接创建/操作 → 保存重启恢复**，不是控制中心创建。此前要求用户在控制中心点击才能继续属于执行偏移，撤销该验收要求。控制中心只作辅助管理；已有启动/载荷修复保留，但不计为桌面右键验收。

当前代码审计：ExplorerCommand 通过 ActivateApplication 发送带坐标/时间/nonce 的创建命令；App 按桌面显示器进入 RequestDesktopWorkspaceCreate，已有独立桌面预览窗口。发现热启动转发后 Program 无条件 TryBringToForeground 主窗口，可能覆盖桌面预览焦点并拉出控制中心；现只对普通激活前置控制中心，创建命令（含拒绝/过期）不走该分支。回归覆盖正常创建、普通激活、无效与过期命令的前置策略，属于工程验证，不冒充 Explorer 真点击。现有预览失败/特定运行时回退仍会进入控制中心，后续必须改为桌面就地可恢复交互，不能作为正常桌面验收通过路径，也不直接删除安全回退。

新的执行顺序：A0 可信安装载荷与扩展审计 → A1 安装后真实桌面右键创建（冷启动和热启动）→ A2 桌面基本操作及重启恢复 → A3 文件夹/引用 → A4 安装生命周期回归 → A5 任务栏 → A6 发布收口。A1 的必要前置是获准签名身份与可丢弃安装环境（#274），不以修改宿主注册表、信任未签名包或伪造菜单绕过。当前本机未安装正式包；这项缺口明确保持未通过，不能再换成控制中心测试作为替代。

本切片验证：完整 Release 1,521/1,521（27 秒、零跳过），App Release 构建零警告/错误，格式、Explorer 命令源码合同、单实例源码合同、文档新鲜度与 diff 检查通过。单实例合同新增对真实 Program 条件接线的检查。未进行安装/物理右键/焦点实测，现有隔离窗口仍为上一发布包，不能用它验证本次新代码。下一工程项是桌面预览失败时的就地反馈与恢复设计，安装身份/环境同步准备；提交与推送不代表 A1 通过。

本节优先于下方历史切片和 PF 扩展顺序。当前唯一主项是 **发布形态的首次创建闭环验收**，不是规则功能、界面微调或扩大探针数量。计划后已开始 A0 实施，实际结果如下；未安装系统组件或取得完整 App 点击通过证据。

#### 审计事实与纠偏

A1 启动修正复测（2026-09-22）：从 `7ef2acd` 完整生成 `LongGrid-0.1.0-a0resources-20260922-win-x64.zip`，SHA-256 `4f77075ed35f638272378170c369df5ead6007e1e702a800563046dab80f3031`，805 文件；全量 1,521/1,521（22 秒）、构建零警告/错误及打包质量门禁通过。解包后用同一隔离启动路径重测，实际得到 `managedLaunchReady=true / productWindowActivated=true / productWindowReady=true / hostWindowTitle=Long方格`，不再启动即退出。旧包缺 PRI 时失败、新包含 PRI 时成功，支持将缺失界面资源认定为本轮发布启动故障的直接修正点，不泛化为所有闪退根因。

当前窗口已留给用户物理输入，配置在 harness 自有临时会话内，不是正常配置；不在仓库记录本机 PID/会话路径。没有调用 computer-use 窗口查询或 UIA，也没有自动点击创建。因二进制 XAML 3.2.3.0 的发布形态外部自动化尚未取得安全证据，本轮该路径保守阻断，非 UIA 启动只证明窗口就绪。**A1 仅启动步骤通过；创建、桌面可见、保存及重启保留仍未验收。** 下一步由当前会话真实点击创建取得结果，或先建立已验证的安全自动化环境，不能跳转 A2。

2026-09-22 A1 首次真实启动失败：为现有隔离会话增加 PublishedPackageRoot（必须 NoBuild），发布包先执行哈希/自包含检查，不把共享 Runtime 包库存当作其运行前提。外部自动化仍对未知或二进制 XAML 3.2.3.0 保守阻断；未使用窗口控制器读取 UIA/截图。首次仅启动修正包，主窗口未就绪即退出 `-1073741189 / 0xc000027b`；同进程本地诊断为 WindowCreating、XamlUnhandled、HResult `0x802B000A`，Windows 事件故障模块确认为包内 Microsoft.UI.Xaml.dll。其文字版本 3.2.0.2511 与二进制 3.2.3.0 不一致，不能凭文字版本判安全。事件不是以往共享 Runtime/UIA 崩溃的充分同因证据。

检查真实载荷发现开发输出有 `LongGrid.App.pri`，发布 ZIP 缺失该界面资源索引；新增强制检查在旧包上失败。打包现显式复制本次构建资源索引并纳入哈希，尚须新包启动复测确认能否消除该失败。启动失败时既有 harness 已清理自己创建的隔离会话，没有删除正常用户配置；本次 A1 明确记为失败，不再写成仅“环境阻断”。

A0 修正包实测：干净提交 `f056be6` 生成 `LongGrid-0.1.0-a0fix-20260922-win-x64.zip`，SHA-256 `438b9905bed3c7cceda3e44c4df7413870f10c62a5c47c3f8a535b158853d60c`，804 文件；完整打包未跳过质量检查，1,521/1,521、构建零警告/错误、格式/覆盖率门禁/漏洞检查通过。解包预检核验 803 个哈希文件并通过新增载荷检查；两个 Worker 已使用 includedFrameworks 8.0.29，不再声明外部 framework。分别无参数启动真实 Worker 都返回预期 64（参数拒绝），未执行任务栏修改或缩略图读取。这只证明本机 Worker 入口可加载，不等于其业务功能、无运行时干净机或主窗口通过。产物与解包目录保留在本机 artifacts（不上传仓库），新电脑应从记录提交重建，不要求重建 ZIP 的哈希跨机器相同。A0 仍需发布形态启动隔离与安全准入核对、元数据口径修正及 MSIX 原生扩展载荷审计；A1 未测，下一步不是 A2。
...
- 当前 MSIX 使用开发身份、未签名；签名合同仍要求获准发布者及受保护签名提供方。生命周期脚本只验证源码合同，非 ValidateOnly 模式明确拒绝执行。包构建成功不是安装成功，更不是升级/卸载验收成功。
- 本机未安装 LongGrid MSIX；源码中的 Explorer `Directory\\Background` 注册依赖安装，开发 EXE/ZIP 不能替代桌面右键入口验收。
- `Pack-LongGrid.ps1` 的产物元数据仍写 `desktopHostExecutionEnabled=false`，与当前产品宿主默认启用的代码存在口径差异；下一实施步必须追踪消费者，区分“未获验收批准”与“运行时禁用”，不得直接翻转为 true 冒充功能通过。
- 方案优先复用 **自包含载荷 + 正式签名 MSIX**。不为解决本机环境先引入第二套安装器。自包含会增加体积，也要求应用更新时维护随包运行时；特殊 API 仍可能有外部包依赖，需逐项验证。依据：[Microsoft 自包含部署说明](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/self-contained-deploy/deploy-self-contained-apps)。

#### 顺序、交付与完成门槛

| 步骤 | 实施内容 | 必须看到的结果 | 当前状态 |
|---|---|---|---|
| A0 发布验收基线 | 集成已通过检查的修正；从明确提交生成自包含产物，审计主程序、两个 Worker、原生扩展及依赖，校准验收入口与元数据 | 产物对应唯一提交/哈希；能明确区分开发运行与发布运行；不能仅靠清单布尔值判定依赖齐全 | 下一步 |
| A1 桌面右键首次创建 | 在获准的干净 Windows 11 x64 环境安装可信 MSIX；桌面空白处右键点击创建，分别覆盖 App 未运行与已运行 | 菜单可见；不打开控制中心也能在桌面命名/确认/取消；确认只生成一个可见盒子，取消零创建；保存并重启保留；不要求脚本或手装依赖 | 未通过；签名与安装环境待准入 |
| A2 基本操作 | 在同一真实盒子上移动、缩放、重命名、折叠/展开、锁定/解锁、删除及撤销；重复保存/重启 | 鼠标键盘可用、位置名称恢复、锁定有效、失败提示可恢复；不移动或删除用户真实文件 | 未通过 |
| A3 文件夹与引用 | 测试绑定/取消/解绑、刷新、打开、Explorer 拖入及盒子间引用调整；使用隔离文件测试权限/路径失效和保存失败 | 文件夹可见、可操作，错误不会闪退，引用与磁盘操作明确区分，文件哈希/位置不变 | 未通过 |
| A4 安装生命周期回归 | A1 已覆盖首次安装/菜单；此处验证升级后菜单、Explorer 重启、单实例、卸载菜单恢复与重装 | 右键命令无重复/失效；卸载后无残留入口；不要求导入开发证书或关闭安全策略 | 未通过 |
| A5 任务栏核心 | 在认证 Windows 构建与可丢弃环境实际应用支持的预设，测试退出、异常恢复及不支持环境提示 | 支持环境能看到效果并恢复原状态；不支持环境不破坏系统、不虚假承诺已应用 | 未通过；有独立平台门禁 |
| A6 发布收口 | 对同一候选包完成干净机在线/离线安装、升级、失败回滚、卸载/重装；完成 DPI/多屏、辅助功能、稳定性和性能验收 | 旧配置升级可用；卸载无残留菜单/后台进程且不误删用户文件；签名、许可、SBOM 与证据全部可追溯 | 未通过 |

A1 不通过不得推进 A2/A3 外围实现；遇到失败只修当前真实阻断并重跑原用例。签名身份和测试环境准备可并行，但不得用发布准备替代 A1。A1 的 ZIP/EXE 只用于验证同源载荷，不是完整正式交付，也不承担右键安装承诺。M1/M2 必须各自达到原里程碑出口，A1 通过不能代替其余核心或 M5。

#### 验收记录与执行纪律

- 每个用例记录提交、产物 SHA-256、OS/build/架构/DPI、部署形态、配置隔离方式、操作步骤、预期与实际、截图或录像、相关有限日志。区分人工操作、UI 自动化、真实文件系统/原生窗口测试和源码合同，禁止互相替代。
- 状态只用“未测 / 阻断 / 失败 / 通过”；通过必须有该用例的真实证据。初次创建、默认交互、持久化丢失或闪退均是当前主项的阻断缺陷。
- 每个修复闭环：复现 → 定位 → 最小修改 → 自动回归 → 原操作重测 → 更新本计划与 PR → 审计并推送 → 远端检查。合并与发布仍需满足各自门禁；未测项不可标为关闭。
- 换机保留脱敏证据索引与产物定位；不上传用户桌面内容、完整配置或未经筛选的崩溃转储。正式用户不需要阅读验收文档或运行诊断脚本才能创建盒子。
- 不使用“已完成多少测试/Stage”换算产品完成百分比；以 A1～A6 的真实结果汇报。当前没有可信的剩余工期估算，先完成 A0/A1 才能据实际缺陷评估。

以下为历史审计记录，不能覆盖上述执行顺序。

2026-09-12 主线同步审计：#359 squash 合并后，#360 在测试项目 Compile 链接处与 `origin/main@ba02e4a` 冲突，最新 `f9370a1` 未报告远端检查；不能将其当作“CI 正在通过”。本轮显式合并主线，同时保留预览连续性和启动诊断源码链接。解决后产品与测试文件内容相对合并前没有变化，完整 Release 重新运行 **1,521/1,521**（14 秒、零跳过）。本轮只关闭集成阻断，不新增功能或提高产品完成度；后续以新提交的 PR CI/CodeQL 为准。原 PRD 三项核心稳定优先顺序不变，实机验收仍需合格 Runtime 环境。

2026-09-12 基础交互显示纠偏：实际原生标题 `ProductDesktopContainerHeaderPresentation.VisualTitle` 使用“折叠 ▸ / 展开 ▾”，同一盒子的 activation 折叠按钮却反向显示。现统一按钮与标题的当前状态箭头；UIA 的展开/折叠动作名称、点击事务、锁定、持久化与文件操作均不改变。绘制入口复用可直接测试的实际 ButtonText 方法，新增两种状态回归，修正前真实 0/2，修正后完整 Release **1,521/1,521**（15 秒、零跳过），App 构建零警告/错误、205-ID UI 合同通过。格式首测发现新方法缩进问题，已修正并重新检查。这是基础操作状态的一致性修复，不是“无法操作”或闪退的根因修复，不增加产品完成度；完整 App 物理验收仍 Pending。

2026-09-12 联合接续审计：PR #360 已整合 PR #359 的启动诊断源码。两个分支新增 App 源码测试链接时发生同一区块冲突，现同时保留两个链接，避免合并后漏测。联合树完整 Release **1,519/1,519**（15 秒、零跳过），App Release 构建零警告/错误，格式与 205-ID UI 合同通过。以下 1,511 与诊断段的分支测试数量均为历史切片记录，不代表联合树测试总数。远端 CI 和 CodeQL 须以联合提交重新验证，推送不等于已经合入 main。

集成更新：PR #359 已在 CI [34676039606](https://github.com/Longyuyeee/Long_Grid/actions/runs/34676039606) 与双语言 CodeQL [34676039597](https://github.com/Longyuyeee/Long_Grid/actions/runs/34676039597) 全部成功后合入 `main@ba02e4a`。PR #360 的预览连续性及本轮箭头修正仍须以自己的最新提交通过远端检查，不借用 #359 结果作为联合通过证明。

接续仍为：在满足 Runtime 准入的电脑完成首次创建→保存→重启恢复、桌面选择/打开/移动/缩放的真实用户旅程；用启动阶段和有限失败代码辅助定位异常，不能把“新增日志”当作“闪退已修好”。本机尚未通过 Runtime 准入，不修改共享 Runtime、不绕过门禁。PF-020B 继续暂停，M1/M2 及产品完成度不因联合测试增加而上调。

历史独立切片：首次整理预览连续性（从已合并 `main@bcc7243` 开发，独立验证时不包含 PR #359 诊断改动）。

- **实际代码问题**：目录/拓扑状态刷新调用 `ApplyProductWorkspaceSessionViews`，界面 `ApplyProductQuickStartSuggestion` 原先无条件隐藏列表并禁用确认按钮，即使前后建议完全一致也会打断用户已打开的预览。
- **修正**：仅在用户已打开且可提交时，比较 edit revision、catalog generation、两组指纹、容器名、候选数量、截断标记和逐项内容/顺序。全部一致时更新内部建议引用但不重建列表、不关闭确认；新生成的 PreviewId 不代表内容变化。任一上述事实变化、建议不可用或无有效 ID 仍走原失效路径，后台刷新不会自动打开预览；提交端原有版本/指纹复核不变。
- **验证边界**：测试链接实际 App 判定源码，覆盖相同输入、新 PreviewId、未打开、无旧建议、全部字段变化以及不可用状态；UI 合同验证正式调用位置在重建列表之前。此为局部交互修复，尚无完整 WinUI 实机录像；初次测试编译的 Folder 枚举名称错误已改为项目实际 Directory。
- **接续**：仍优先基础创建/操作/保存重启及闪退定位，PF-020B 不恢复。PR #359 原独立处理诊断能力，现已整合并以上方联合审计为准。

本切片最终实测：完整 Release 1,511/1,511（16 秒、零跳过），App Release 构建零警告/错误，格式、205-ID UI 合同、文档新鲜度和 diff 检查通过。相对 `bcc7243` 新增 3 个预览连续性测试；测试数量不与未合并诊断分支混算。代码与审计一同推送独立 PR，实机证据仍 Pending。
...
2026-09-12 后续切片：默认桌面交互正式接入（前两条交互缺口记录为修复前事实）。

- **需求与方案**：对齐 PRD 核心任务 1 和“容器内项目支持键盘和鼠标操作”。新增明确的 `EnabledForProduct` 策略状态，App 三层组合改用 `EvaluateForProduct`，在没有遗留环境覆盖时允许显示显式交互入口；用户仍需点击盒子现有的 ↗ 按钮进入交互。不是把人工验收环境变量写成 1，也不是启动即夺取焦点或开放整个桌面捕获。
- **保留边界**：宿主关闭、紧急禁用、显式 legacy 覆盖仍优先；旧 `Evaluate` 继续默认关闭，显式开发配置仍要求原人工会话准入。输入来源校验、窗口权属、代次、锁定、失焦/ESC/系统切换取消和真实文件安全规则不变。UI 合同明确要求 App 使用产品组合，防止回退为测试手动启用而正式入口关闭。
- **实测**：产品策略和生命周期专项 60/60，包括默认状态仍为 Passive/无租约、准入后选择/打开、重复消费拒绝与失焦取消（使用 surface/activation 替身）。另增真实 Windows 原生窗口测试 1/1：生产默认三层组合实际创建 HWND 与 activation source，Passive 合同通过、键盘交互入口可请求、没有自动进入 Explicit；测试关闭时释放窗口。此测试不加载 WinUI，不代表物理点击或完整 App 启动通过。App Release 构建 0 warning / 0 error、205-ID UI 合同与文档新鲜度通过；最终全量测试和格式结果随提交记录。
- **剩余问题与下一步**：入口的 ↗ 可发现性、默认模式真实鼠标/键盘、移动/缩放、保存/重启、关闭/恢复须在合格环境验收；本机 WinUI Runtime 门禁未关闭，闪退仍未定位。不得把本切片标为整体基础操作完成；不恢复 PF-020B 扩展。

最终本地回归：Release 1,499/1,499（14 秒、零跳过）；新增默认产品策略没有替换或删减原开发模式用例。源代码、测试与本节审计一同推送 PR #358；GitHub 远端检查完成前不声称已合入 main。

2026-09-12 后续切片：文件夹绑定选择器的可恢复异常边界。

- **实际缺陷**：`ProductWorkspaceFolderBindButton_Click` 为 `async void`，选择器构造、HWND 初始化、异步选择和读取 Path 均没有局部异常边界。Shell/WinRT 抛出 COM 或权限异常时会离开点击处理器；原实现也未阻止等待期间再次启动选择器。这是明确的异常处理缺口，但尚未复现用户报告的闪退，不能称为其确定根因。
- **修正与需求对齐**：对应 PRD 文件夹绑定核心的取消零写入、错误可恢复要求。App 内提取 `ProductFolderPickerOperation`，将 COM/权限失败映射为 Unavailable，将正常取消和取消异常映射为 Cancelled；Busy 不调用第二个选择器。只在成功路径继续既有稳定身份探测和带 expected revision 的绑定提交。界面显示可重试提示，不显示可能含私人路径的异常消息；不全局吞异常、不替换配置、不增加文件操作。
- **测试**：链接真实 App 操作源码，覆盖同步/异步 COM 失败后重试、权限拒绝、两种取消、重复调用只打开一次，以及未知异常继续抛出且释放 Busy。首轮完整测试 1,503/1,504：旧 UI 合同要求 `folder is null` 导致预检用例失败；已改为检验有限状态的取消/失败在提交之前返回，未删除该保护。App Release 构建零警告/错误、格式通过；最终全量与 UI 合同重跑后补录。
- **接续**：仍优先完整启动、默认移动/缩放与保存重启；选择器物理点击与异常环境也须实机验收。WinUI 运行时、具体闪退堆栈、安装右键等未关闭项不因本修复转为通过，PF-020B 继续暂停。

本切片最终结果：Release 全量 1,504/1,504（14 秒、零跳过），205-ID UI 合同与文档新鲜度通过；代码、测试和审计一起推送 PR #358。以上没有新增真实 WinUI 操作或崩溃复现证据。

2026-09-12 后续切片：首次整理保存失败自动补偿。

- **根因与修复**：`CommitQuickStart` 返回整批 `CompensationToken`，但 App 原先只发布新配置，不跟踪其保存结果；旧失败测试直接手动调用 undo，未证明产品自动补偿。现由 App 的 `ProductQuickStartSaveCompensation` 跟踪 edit/save revision，匹配的 Failed 才调用既有整批撤销并移除历史项；先消费令牌再执行补偿，防止重入和重复通知。成功保存、后续编辑或后续保存版本都终止旧回退资格。
- **发布与提示**：接受后在 Dispatcher 再观察当前保存快照，覆盖保存过快导致事件先于 Track 的情况；补偿后同步工作区、桌面投影、首次整理失败提示，并取消首次旅程完成标记。复用既有撤销保存链，不移动文件；补偿本身保存失败仍沿原保存重试机制处理。
- **验证边界**：测试链接实际 App 观察器，覆盖失败后整批回退、重复通知零重复提交、文件哈希不变及三种过期/成功通知保护；没有新增物理 WinUI 或崩溃复现证据。首轮新增用例清理因尚有待重试状态被 Dispose 拒绝，修正为完成测试保存后退出；中间一次 Submit 类型错误也已修正，最终结果以下方实测为准。
- **下一步**：继续基础用户旅程验收与闪退定位，不恢复 PF-020B。默认移动/缩放与持久化尚未取得完整 App 物理操作证据。

本切片最终实测：Release 1,507/1,507（14 秒、零跳过），App Release 构建 0 warning/error，格式、205-ID UI 合同和文档新鲜度通过。App 观察器使用 SaveController 当前权威快照而非 Dispatcher 队列里的旧事件，避免重试已成功却被旧 Failed 通知回退。代码和审计继续推送 PR #358，远端检查与合并结果另行确认。

2026-09-12 后续纠偏：补偿保存的重试反馈与过期通知。

- **审计发现**：上一切片首次整理回退复用“正在保存撤回状态”的提示，强制隐藏重试；若补偿保存同步失败或已完成，提示会覆盖真实保存状态。且只有 Quick Start 读取最新快照，其他分支仍使用排队的旧事件，可能显示过期状态。
- **修正**：首次整理回退提示改为接收当前保存快照，保存栏完全复用正式保存状态渲染（失败显示重试，成功恢复相关操作）；首次整理说明独立保留。统一在 `ApplyProductWorkspaceSaveSnapshot` 入口读取当前 SaveController 快照，旧事件仅充当刷新信号，不再以历史失败触发回退或覆盖新状态。
- **验证**：新增连续两次失败用例：首次保存失败 → 整批补偿 → 补偿保存失败且 CanRetry → Retry 成功，仍为空工作区、历史为空、文件哈希不变，不重复补偿。UI 合同检查正式 App 的当前快照刷新和 Quick Start 真实保存状态渲染接线；这仍是代码合同，不是实际屏幕点击证据。
- **边界**：本修正不改变原始 PRD，不推进外围功能；物理操作、闪退根因、运行时与分发门禁仍未关闭。提交前补录最终回归结果。

本切片最终实测：完整 Release 1,508/1,508（17 秒、零跳过），App Release 构建零警告/错误，格式、205-ID UI 合同、文档新鲜度与 diff 检查通过。已审计修正前后的重试入口和旧通知处理；随代码推送 PR #358，不把远端 Pending 标为通过。

2026-09-12 后续补齐：桌面直接创建回退的保存反馈。

- 复查 `ApplyProductWorkspaceCreateSaveRollbackState` 的全部两个 App 调用点（整批恢复令牌与普通新盒子移除）发现，它们仍传入失败原因和版本号，而不是回退保存的真实状态；方法无条件隐藏重试并显示“正在保存”，遗漏了前一切片首次整理以外的同类问题。
- 两个调用点现均传入 SaveController 当前快照，先复用正式保存栏渲染；仅 WaitingForDebounce/Saving 时显示正在保存回退状态的提示。Failed、Saved、Clean 等状态不再被覆盖，重试和导入导出可用性完全按真实保存状态决定。不改文件操作、提交/补偿事务或原始 PRD。
- UI 合同新增该正式方法的状态分支检查，防止仅修 Quick Start 却再次遗漏桌面直接创建；此合同是源码接线检查，不冒充实机 UI。App Release 构建零警告/错误，205-ID UI 合同与 diff 检查通过；最终全量和格式结果随提交记录。
- 尚未关闭：完整 App 的创建/移动/缩放/保存/重启物理验收、闪退根因与运行时门禁。当前开发仍是基础可用性修复，不恢复 PF-020B，不宣称软件收尾。

本次最终结果：Release 1,508/1,508（14 秒、零跳过），格式检查与文档新鲜度通过。与上述构建、UI 合同结果一同审计后推送 PR #358；远端结果尚待确认。

2026-09-12 集成结果：PR #358 已合入 `main@bcc7243`；CI [34675331371](https://github.com/Longyuyeee/Long_Grid/actions/runs/34675331371) 为 1,508/1,508（41 秒），CodeQL [34675331355](https://github.com/Longyuyeee/Long_Grid/actions/runs/34675331355) 双语言通过，测试/覆盖率产物 ID `10292371538`。以上各切片的 Pending 是提交时快照，不再表示该 PR 未合并。完整产品验收仍未完成。

后续切片：启动与异常的有限本地诊断，输入基线 `main@bcc7243`。

- **实际问题**：原入口在单实例转发失败时只返回 1；WinUI 未处理异常和初始化 Task 失败缺少通用诊断记录。无法将用户报告的“闪退”定位到入口、窗口构造或工作区初始化，也难以换机传递。此切片直接服务当前崩溃定位，不增加外围功能，不宣称修复了未知崩溃。
- **实现**：启动阶段写入固定 `latest-startup.json`，已观察到的失败写入独立 `latest-failure.json`，后续正常启动不清空失败记录。字段仅 schema、RunId、PID、UTC、阶段、有限异常类别、来源和 HResult；不记录路径、文件名、命令行、异常消息/堆栈或 Exception.Data，不上传。每份内容最多 4 KiB、固定两份，不累积日志；现有重解析路径跳过，IO/权限失败不覆盖原程序结果。
- **行为边界**：入口及未预期初始化异常记录后仍抛出；WinUI 事件不设置 Handled，原有可恢复初始化分支仍激活主窗口，单实例转发仍返回原失败码。未改动系统运行时或产品配置。原生 fail-fast、进入 Main 之前的加载失败、进程被强杀，以及写盘被中断不保证留下完整 JSON；记录缺失不代表正常退出。多进程竞争时是 best-effort 最新快照，需结合 RunId/PID/UTC 区分旧故障和当前运行。
- **换机取证**：出现问题后复制 `%LOCALAPPDATA%\LongGrid\diagnostics\startup\` 内两份 JSON（存在才复制），同时记录使用的版本/提交、启动方式、最后点击的操作和发生时间。不要把整份产品配置或桌面文件一起上传。没有失败记录时继续查 Windows 应用事件/WER；不能把最后阶段自动认定为根因。
- **验证**：链接实际 App 日志源码的测试覆盖内容去敏、HResult/阶段、固定大小/文件数、跨运行保留失败、目标不可写不影响原结果。完整 Release 1,511/1,511（20 秒），App 构建 0 warning/error，205-ID UI 合同与 diff 检查通过；格式与文档检查提交前确认。初次编译发现两个 UnhandledExceptionEventHandler 命名冲突，已显式限定 System 类型后通过。
- **下一步**：在获准的合格运行时环境重放真实用户旅程，使用记录定位具体异常，再针对性修复；不可把本轮诊断能力当作实机验收或闪退已修复。PF-020B 继续暂停。

本切片提交前确认：格式、文档新鲜度、单实例源码合同和 Release 启动链 ValidateOnly 均通过；后两者没有启动真实 App，不新增实机验收结论。已将故障记录边界和换机传递方式写入本节，随代码推送独立 PR。

后续审计纠偏：诊断输出路径链接检查（PR #359）。

- 原写入只在 Exists 为 true 时检查 ReparsePoint，忽略了“链接自身存在但目标不存在”的元数据情形；现同时检查 LinkTarget，文件目标和目录祖先共用判断。普通不存在的路径仍允许创建，已知链接或重解析路径直接跳过记录。
- 初次真实文件符号链接测试因 Windows 返回“客户端没有所需的特权”而失败，**没有**复现越界写入，也没有提升权限或打开开发者模式。改为五个明确的元数据回归，覆盖悬空链接、有效链接、重解析属性、普通缺失路径与普通文件；原真实文件记录/大小/隐私/不可写回归仍运行。真实链接系统调用端到端证据继续缺失，不宣称对路径竞争替换或硬链接的完整防护。
- 完整 Release 1,516/1,516（14 秒、零跳过），App 构建零警告/错误，205-ID UI 合同与 diff 检查通过；格式、文档检查提交前确认。此切片仅修正本轮诊断实现的边界，不改变原始产品范围，也不代表已解决闪退或完成用户旅程验收。

提交前格式与文档新鲜度检查均通过；代码、测试和上述限制一同推送 PR #359，远端结果以该 PR 为准。
