...
配置层位置与尺寸已按有限 DIP 预设进入[正式容器受限布局预设提交审计](57-container-bounded-placement-commit-audit.md)。显示器归属、最小可见面积和真实窗口应用仍需权威拓扑解析与恢复预览。容器删除、跨动作撤销、DesktopHost 接线和真实文件整理继续独立准入。
