...
| P0-03 | 图标/缩略图 | 主进程 500 项有界加载/句柄闭环；真实零 Capability AppContainer worker、挂起后先入 Job、最小句柄继承、协议 v6 的副本/最小路径 ACL 对照、未代理读写阻断、正常 ACL 恢复、硬超时/Profile 清理和有界共享内存 BGRA32 IPC 已测；26100 两种输入均安全拒绝；正式渲染和真实 provider 矩阵待验证 | 完整矩阵前只显示类型图标/缓存缩略图，现场提取保持实验能力 |
