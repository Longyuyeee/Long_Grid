...
P0-03a 当前实测：96 个真实桌面项目预热成功，三轮各 500 次后台图标提取全部成功；并发峰值为 4，成功 `HBITMAP` 与 `DeleteObject` 释放数严格一致，预热后 GDI 句柄净增量均为 0。缓存缩略图仅命中 1/13，这属于 `SIIGBF_INCACHEONLY` 的预期语义。P0-03b 先用 Low Integrity 对照证明 MIC no-write-up 不能阻止读取其他用户可读文件，随后把真实 worker 迁入零 Capability AppContainer。协议 v6 对照最大 32 MiB、拒绝重解析点的 `ControlledCopy` 与 `MinimumPathAcl`，未代理相邻读取均被拒绝，正常 ACL lease 结束后随机 SID 显式 ACE 被复核清除；副本仍为默认实验。格式矩阵现在对 BMP/PNG/GIF/JPEG/TIFF-RGB/TIFF-LZW/HEIC/AVIF 运行普通父进程 Shell 基线，再执行两种 worker 策略，并只记录脱敏 handler 健康与 HEVC/AV1 decoder 能力布尔值。Windows 22621 可枚举 HEVC、不可枚举 AV1：前四格式在父进程和 worker 均成功，两种 TIFF 在三条路径均精确 `0x8007007E` 且 handler 已注册但模块缺失，HEIC/AVIF 在三条路径均精确 `WTS_E_FAILEDEXTRACTION (0x8004B200)`；默认副本压力 500/500、p95 29.60 ms。Windows 26100 CI 的 HEVC/AV1 均不可枚举；父进程前六项成功、HEIC/AVIF 精确 `0x8004B200`，十六个 worker 组合全部输入可读并精确返回 `E_ACCESSDENIED`。这证明 handler、decoder 和 Shell 实际提取是三层不同证据。像素继续使用最大 262,144 bytes 的匿名页文件映射和单请求不可继承句柄；250 ms 强制卡死、连续超时退避、协议/像素故障、父进程无清理退出及 Profile 清理均通过。正式渲染、HEIC/AVIF 成功环境、Office/PDF、云/网络、受控第三方 provider/ARM64 矩阵、干净 TIFF 环境和另一种 stream/decoder 合同仍未关闭，详见[P0-03b 报告](spikes/P0-03b-thumbnail-worker-isolation.md)。
...
| P0-03 | 图标/缩略图 | 主进程 500 项有界加载/句柄闭环；真实零 Capability AppContainer worker、协议 v6 的副本/最小路径 ACL 对照、八样本父进程基线与双 build 十六组合、脱敏 handler/decoder 健康、未代理读写阻断、正常 ACL 恢复、硬超时/Profile 清理和有界共享内存 BGRA32 IPC 已测；两 build 的 HEIC/AVIF 父进程均精确提取失败；26100 worker 全部安全拒绝 | 完整矩阵前只显示类型图标/缓存缩略图，现场提取保持实验能力 |
