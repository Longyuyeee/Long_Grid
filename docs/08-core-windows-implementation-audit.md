...
| P0-03 | 图标/缩略图 | 主进程 500 项有界加载/句柄闭环、受限 Low Integrity worker、挂起后先入 Job、最小句柄继承、硬超时恢复、显式句柄 broker 的有界共享内存 BGRA32 IPC 和写阻断已测；文件 broker、正式渲染集成和真实 provider 矩阵待验证 | 完整矩阵前只显示类型图标/缓存缩略图，现场提取保持实验能力 |
