...
2. `RecoveredFromBackup` / `SafeMode` 状态 UI、恢复动作与当前 v1 受限导入已由后续切片接入，仍缺 schema 迁移、导出和历史证据生命周期；
...
因此本切片是 **E3 / Production lifecycle slice pass**，Issue #24 继续保持 OPEN。后续已实现配置恢复状态 UI、经确认的恢复动作和当前 v1 受限导入，下一项是 schema 迁移/导出/证据生命周期或真实产品状态入队；不得把本次匿名启动参数测试描述为文件、插件或小组件协议已兼容。
