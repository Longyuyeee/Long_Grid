...
- 14 项定向测试覆盖完整强样本、7 类单点降级、空/非法样本、非 Windows、真实 Windows 有限采样与身份脱敏、generation/latest-wins、取消和关闭排空；
- 真实 Windows smoke 通过正式 Reader 调用原生源，要求结果有限、非 Unsupported，并验证所有返回 StableId 均为 64 位十六进制摘要；它不把当前机器样本状态冒充多屏/热插拔验收；
- Debug/Release 全量构建均为 0 warning / 0 error，测试 362/362 通过；14 项拓扑定向测试全部通过；
- 覆盖率为行 90.54%（5533/6111）、分支 82.67%（1197/1448），通过仓库 90%/75% 门槛；Windows P/Invoke 生产实现未使用覆盖率排除属性；
