...
- 应用关闭排空、单实例激活、恢复状态 UI、经确认的备份接受与 SafeMode 空白安全重置已有生产切片证据；外部导入/导出和历史证据清理留在后续切片；
...
首个 `LongGrid.Infrastructure` 适配器、latest-wins/App 有界关闭排空、完整单实例、恢复状态 UI、显式备份接受与 SafeMode 空白重置已建立；下一步是外部导入合同或真实产品状态入队。专用环境真实卷矩阵可独立执行，全部证据链完成后才能关闭 Issue #24。正式程序集继续不得引用或改名发布 spike 程序集。
...
当前自动证据覆盖：首次原子发布、第二次替换保留上一版备份、损坏主文件回退备份、主备均不可用时进入安全模式、普通保存拒绝覆盖损坏证据、跨进程文件租约有界超时与调用方取消、I/O 错误不在公开异常中泄露路径、latest-wins/关闭排空合同、单实例源码合同、四态恢复 UI 合同、确认后原子接受备份并归档损坏主配置，以及确认后发布标准空白配置、分别归档主备证据、发布失败回滚和中断标记重试；本机另有真实双进程转发和最小化恢复证据。该增量允许产品代码依赖正式 Infrastructure 与 AppLifecycle 边界，但不改变 I24-01/I24-02 的 `PendingDedicatedEnvironmentEvidence`，也不代表外部导入、真实断电或非 NTFS 耐久性已经完成。
