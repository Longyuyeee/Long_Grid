...
后续 PF-002C2 已关闭“首个方格后入口消失”的工程缺口：非空工作区在每个权威显示器提供避让现有方格的持续入口，主显示器保持唯一快捷键，见 [Stage 158](158-pf002c2-persistent-desktop-create-audit.md)。PF-002 仍因就地预览、保存补偿和物理输入证据未完成而不能升级。

## 8. 严格后续开发顺序

1. **PF-002C2（已完成）**：非空工作区持续入口、多显示器就近目标、唯一主屏快捷键、空位失败关闭和 Explicit 互斥已形成工程合同，见 [Stage 158](158-pf002c2-persistent-desktop-create-audit.md)。
2. **PF-002D（下一步）**：提交前就地预览与命名。预览名称、显示器、位置和尺寸；支持键盘/UIA 编辑；Esc、失焦、安全门关闭、revision/topology 变化必须取消且零残留。
