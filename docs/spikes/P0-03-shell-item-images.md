...
- P0-03b 已把真实工作进程迁入零 Capability AppContainer，验证挂起后先入 Job、最小句柄继承、32 MiB 受控只读输入、协议 v6 的副本/最小路径 ACL 对照、未代理读写阻断、ACL/Profile 清理和有界共享内存 BGRA32 IPC；BMP/PNG/GIF/JPEG/TIFF 双 build 十组合矩阵在 22621 观察到前四格式成功、TIFF 精确 provider/module 缺失，在 26100 则全部输入可读但 Shell 安全拒绝，正式渲染集成和其余真实 provider 矩阵仍未完成；
...
2. 继续 P0-03b：在五格式基线上扩展 HEIF、Office/PDF、云/网络和第三方 provider，并定位 TIFF 模块依赖；handle-backed 输入改为独立 stream/decoder 或 Shell provider 合同实验，再决定正式渲染表面集成；
