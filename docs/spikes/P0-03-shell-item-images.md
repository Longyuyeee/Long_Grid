...
- P0-03b 已实现并压力验证受限 Low Integrity 可回收工作进程、挂起后先入 Job、最小句柄继承、显式复制匿名映射句柄的有界 BGRA32 IPC 和 worker 写阻断；文件 broker、正式渲染集成和真实 provider 矩阵仍未完成；
...
2. 继续 P0-03b：比较 AppContainer/受限 token 与文件 broker 边界，再推进正式渲染表面集成；实际 Low Integrity worker、共享内存句柄 broker、有界 BGRA32 IPC、写阻断与父进程 PID/Job Object 双重退出清理已有探针证据；
