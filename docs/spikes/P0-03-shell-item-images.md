...
- P0-03b 已把真实工作进程迁入零 Capability AppContainer，验证挂起后先入 Job、最小句柄继承、32 MiB 受控只读输入副本、协议 v5 transport 门禁、未代理读写阻断、Profile 清理和有界共享内存 BGRA32 IPC；brokered handle/最小路径 ACL 比较、正式渲染集成和 provider 矩阵仍未完成；
...
2. 继续 P0-03b：比较单请求句柄、当前受控副本和最小路径 ACL，补齐原路径/provider 兼容矩阵后再推进正式渲染表面集成；
