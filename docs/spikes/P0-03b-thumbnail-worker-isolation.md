...
结果：**Conditional Pass（真实零 Capability AppContainer worker、七个自有格式/编码样本的父进程与双 build 输入策略矩阵、有界共享内存 BGRA IPC、未代理读写阻断、ACL/Profile 清理和合成 500 项预算通过）**
...
父进程在 `%TEMP%` 的随机专用目录生成 BMP、PNG、GIF、JPEG、未压缩 RGB TIFF、LZW TIFF 和 HEVC 压缩 HEIC 七个确定性自有样本；500 次压力仍只重复访问自有 BMP。矩阵先在普通父进程中调用相同 Shell API，并只记录固定格式标签、HRESULT、尺寸及扩展级 handler 注册/模块健康布尔值；不输出 handler 身份、CLSID、厂商或路径。随后 Client 创建随机 AppContainer Profile，把当前 worker 运行时暂存到受 128 MiB 总量约束的 Profile 私有目录，并以两种授权策略逐项对照。
...
| 输入 | 临时沙箱内自有 BMP/PNG/GIF/JPEG/TIFF-RGB/TIFF-LZW/HEIC；500 次压力使用 2×2 BMP |
| 预热 | 首次提取成功后再采空闲 CPU |
| 压力请求 | 500 |
| 每进程请求预算 | 100 |
| 普通请求硬超时 | 2,000 ms |
...
| 自有格式矩阵 | 父进程基线 + BMP/PNG/GIF/JPEG/TIFF-RGB/TIFF-LZW/HEIC × `ControlledCopy`/`MinimumPathAcl`；逐项复核输入可读、Shell 成功或精确安全拒绝、同格式策略一致、ACL/Profile 清理 | 父进程 7/7、worker 14/14 必须精确安全分类；不读取用户文件 | Pass |
...
| 环境 | 受控副本 | 最小路径 ACL | 判定 |
|---|---|---|---|
| Windows `10.0.22621` x64 本机 | 父进程 BMP/PNG/GIF/JPEG 成功；TIFF-RGB/TIFF-LZW 精确 `0x8007007E` 且 handler 已注册但模块缺失；HEIC 精确 `0x8004B200` 且 handler 已注册、模块存在 | 两种 worker 策略与父进程逐格式完全一致；14/14 安全分类；500/500；p95 33.43 ms；ACL/Profile 清理 | `ExtractionSupported` + 精确安全降级 |
| Windows `10.0.26100` x64 GitHub runner | 父进程前六项成功；HEIC 精确 `0x8004B200` 且 handler 已注册、模块存在 | 两种 worker 策略 14/14 输入可读，Shell 全部精确 `0x80070005`；ACL/Profile 清理 | `AccessDeniedSafely` / `ProductFallbackRequired` |

矩阵要求普通父进程每项只能成功、精确返回 `ERROR_MOD_NOT_FOUND (0x8007007E)`，或仅对受控 HEIC 样本精确返回 Windows SDK 定义的 `WTS_E_FAILEDEXTRACTION (0x8004B200)`；worker 还可精确返回 `E_ACCESSDENIED`。同一样本的两种授权策略必须得到完全相同的分类和 HRESULT；worker 是否逐格式匹配父进程、跨格式是否一致都作为诊断证据。22621 上两种 TIFF 编码在父进程和 worker 中都命中陈旧 handler 注册并精确模块缺失；两 build 的 HEIC 父进程都在 handler 已注册且模块存在时精确返回 `0x8004B200`，因此不能仅凭注册状态宣称可提取。26100 的 14 个 worker 调用全部输入可读又被 `0x80070005` 拒绝，把该环境差异定位到 AppContainer/Shell 边界。安全失败都不允许回退到主进程或 Low Integrity 现场提取；产品只能显示类型图标或已验证缓存。
...
扩展共享内存与生命周期矩阵后的代表轮次由主探针启动压力/预算回收、强制超时、协议/像素故障和恢复 worker；另由独立父进程宿主验证孤儿清理，并由父进程基线与两个短命 client 跑七个基线及十四个格式/策略组合。报告不输出路径、文件名、图像字节、句柄值、handler 身份、CLSID、厂商或模块路径；为区分跨 Windows build 的文件授权与 provider 失败，只保留固定格式标签、布尔分类和 HRESULT。
...
- 两个 build 的 HEVC HEIC 父进程都在 handler 已注册且模块存在时精确返回 `WTS_E_FAILEDEXTRACTION`，说明注册健康不等于提取能力；
- 26100 父进程前六样本成功、HEIC 精确提取失败，两个 AppContainer 策略的十四次调用则全部输入可读并精确返回 `E_ACCESSDENIED`；
...
- 500 次是对同一自有 BMP 的隔离/生命周期压力；七个自有格式/编码样本也不等于 500 个不同真实项目的 provider、缓存和渲染预算；
- 尚未覆盖 AVIF、可成功解码 HEIC 的受控环境、Office/PDF、OneDrive、网络路径、受控安装的第三方 provider、恶意文件、x64/ARM64 与完整支持 Windows build 矩阵；本轮只读识别注册健康，不修改用户关联或注册表；
...
1. 在专用干净环境扩展 build × provider 矩阵到 AVIF、HEIC 成功解码环境、Office/PDF、OneDrive、网络路径和受控安装的第三方 provider，并复测 TIFF 默认 handler，保留类型图标/缓存安全回退；
