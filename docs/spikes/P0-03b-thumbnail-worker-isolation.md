...
结果：**Conditional Pass（真实零 Capability AppContainer worker、八个自有格式/编码样本的父进程与双 build 输入策略矩阵、脱敏 HEVC/AV1 decoder 能力预检、有界共享内存 BGRA IPC、未代理读写阻断、ACL/Profile 清理和合成 500 项预算通过）**
...
父进程在 `%TEMP%` 的随机专用目录生成 BMP、PNG、GIF、JPEG、未压缩 RGB TIFF、LZW TIFF、HEVC 压缩 HEIC 和 AV1 压缩 AVIF 八个确定性自有样本；500 次压力仍只重复访问自有 BMP。矩阵先用 Media Foundation 查询 HEVC/AV1 video decoder 是否可枚举，再在普通父进程中调用相同 Shell API；报告只记录固定格式标签、HRESULT、尺寸、扩展级 handler 注册/模块健康和 decoder 可用性布尔值，不输出身份、CLSID、厂商或路径。随后 Client 创建随机 AppContainer Profile，把当前 worker 运行时暂存到受 128 MiB 总量约束的 Profile 私有目录，并以两种授权策略逐项对照。
...
| 输入 | 临时沙箱内自有 BMP/PNG/GIF/JPEG/TIFF-RGB/TIFF-LZW/HEIC/AVIF；500 次压力使用 2×2 BMP |
| 预热 | 首次提取成功后再采空闲 CPU |
| 压力请求 | 500 |
| 每进程请求预算 | 100 |
| 普通请求硬超时 | 2,000 ms |
...
| 自有格式矩阵 | HEVC/AV1 decoder 能力预检 + 父进程基线 + BMP/PNG/GIF/JPEG/TIFF-RGB/TIFF-LZW/HEIC/AVIF × `ControlledCopy`/`MinimumPathAcl`；逐项复核输入可读、Shell 成功或精确安全拒绝、同格式策略一致、ACL/Profile 清理 | decoder 查询成功；父进程 8/8、worker 16/16 必须精确安全分类；不读取用户文件 | Pass |
...
| 环境 | 受控副本 | 最小路径 ACL | 判定 |
|---|---|---|---|
| Windows `10.0.22621` x64 本机 | HEVC decoder 可枚举、AV1 不可枚举；父进程 BMP/PNG/GIF/JPEG 成功，TIFF 两项精确 `0x8007007E`，HEIC/AVIF 精确 `0x8004B200` | 两种 worker 策略与父进程逐格式完全一致；16/16 安全分类；500/500；p95 29.60 ms；ACL/Profile 清理 | `ExtractionSupported` + 精确安全降级 |
| Windows `10.0.26100` x64 GitHub runner | HEVC/AV1 decoder 均不可枚举；父进程前六项成功，HEIC/AVIF 精确 `0x8004B200` | 两种 worker 策略 16/16 输入可读，Shell 全部精确 `0x80070005`；ACL/Profile 清理 | `AccessDeniedSafely` / `ProductFallbackRequired` |

矩阵先要求 Media Foundation 启动以及 HEVC/AV1 decoder 查询成功，再要求普通父进程每项只能成功、精确返回 `ERROR_MOD_NOT_FOUND (0x8007007E)`，或仅对受控 HEIC/AVIF 样本精确返回 Windows SDK 定义的 `WTS_E_FAILEDEXTRACTION (0x8004B200)`；worker 还可精确返回 `E_ACCESSDENIED`。同一样本的两种授权策略必须得到完全相同的分类和 HRESULT。22621 能枚举 HEVC decoder，但 HEIC Shell 提取仍失败，证明 decoder 存在也不是端到端能力保证；AV1 不可枚举与 AVIF 失败则一致。26100 两种 decoder 均不可枚举，父进程 HEIC/AVIF 失败，16 个 worker 调用又全部在输入可读后被 `0x80070005` 拒绝。安全失败都不允许回退到主进程或 Low Integrity 现场提取；产品只能显示类型图标或已验证缓存。
...
扩展共享内存与生命周期矩阵后的代表轮次由主探针启动压力/预算回收、强制超时、协议/像素故障和恢复 worker；另由独立父进程宿主验证孤儿清理，并由父进程基线与两个短命 client 跑八个基线及十六个格式/策略组合。报告不输出路径、文件名、图像字节、句柄值、handler/decoder 身份、CLSID、厂商或模块路径；只保留固定格式标签、布尔分类和 HRESULT。
...
- 22621 可枚举 HEVC decoder 但 HEIC 仍精确返回 `WTS_E_FAILEDEXTRACTION`；decoder 和 handler 健康都不等于 Shell 提取能力；
- 两个 build 的 AVIF 与 HEIC 父进程都精确返回 `WTS_E_FAILEDEXTRACTION`；26100 两种 AppContainer 策略的十六次调用全部输入可读并精确返回 `E_ACCESSDENIED`；
...
- 500 次是对同一自有 BMP 的隔离/生命周期压力；八个自有格式/编码样本也不等于 500 个不同真实项目的 provider、缓存和渲染预算；
- 尚未覆盖可成功解码 HEIC/AVIF 的受控环境、Office/PDF、OneDrive、网络路径、受控安装的第三方 provider、恶意文件、x64/ARM64 与完整支持 Windows build 矩阵；本轮只读识别注册与 decoder 健康，不修改用户关联、codec 或注册表；
...
1. 在专用干净环境扩展 build × provider 矩阵到 HEIC/AVIF 成功解码环境、Office/PDF、OneDrive、网络路径和受控安装的第三方 provider，并复测 TIFF 默认 handler，保留类型图标/缓存安全回退；
