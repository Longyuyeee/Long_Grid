...
5. 建立 500 请求延迟、CPU、内存、句柄和空闲 CPU 的首个预算；
6. 不对真实桌面、云文件、网络文件或第三方提供程序触发现场提取。
...
本轮共启动 7 个 worker：初始/预算回收进程、强制超时进程和恢复进程。报告只输出聚合指标，不输出路径、文件名、图像字节、HRESULT 或 Shell 身份。
...
- 尚未覆盖崩溃、畸形响应、超大响应、协议版本不匹配、父进程退出和孤儿 worker 回收；
