...
第三轮 CI 的 1305 项测试通过且 branches 提升为 `75.68%`，但 lines 仍为 `89.98%`。下载 CI Cobertura 产物后发现主测试中的强杀子进程继承了覆盖收集环境，额外生成一份只执行单个等待测试的低覆盖报告，覆盖脚本按设计聚合两份报告。本机与 runner 因子进程时序不同而出现比率差异。仅移除已知 profiler 变量仍会产生第二报告，因此最终改为给证据子进程建立 PATH/SystemRoot/TEMP/DOTNET_ROOT/USERPROFILE 最小环境，再加入两个专用证据变量；父测试仍受覆盖收集，子进程仍是真实独立 .NET/VSTest 进程，产品断言和强杀行为不变。

第四轮 CI 证明最小环境已消除低覆盖子进程报告：两份上传路径中的 Cobertura 数据完全相同，均为主测试 `22925/25477`，不再是单测试低覆盖数据；但 runner 对条件分支的实际执行路径仍使 lines 为 `89.98%`，距离门槛 5 行。没有删除真实强杀测试或降低门槛，补充确定性的乱序事务调用测试，覆盖四个状态机 no-op 防线以及审计字段读取；这些也是防止上层错误调用绕过恢复事务的产品边界。

回归门禁预期格式无差异、Release 构建零告警/零错误、完整测试零失败；实际 `dotnet format --verify-no-changes` 退出码 0、全解决方案 `0 warning / 0 error`。补齐覆盖边界后完整核心测试 `1306/1306`，本地隔离覆盖率 lines `90.30%`、branches `75.86%`，均高于 `90% / 75%` 门槛；最终结论仍以本提交的 GitHub runner 复验为准。
