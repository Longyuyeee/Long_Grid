...
P0-07a 当前静态证据：双屏有效 DPI 为 192/240，三轮各 100 次枚举均只产生一个拓扑指纹，虚拟屏幕边界与显示器 Bounds 外接矩形一致且无原生资源增长。Core 指纹已经做到枚举顺序和虚拟桌面整体平移无关。生产仍需通过 `QueryDisplayConfig` 补齐 adapter/target、旋转和连接路径，并通过热切换矩阵后才能启用自动布局恢复。
...
| 多显示器/DPI | 静态双屏混合 DPI 指纹已测；100%–400%、负坐标、拔插/旋转/休眠后不丢失布局仍需验证 |
