...
第一组容器创建/重命名已由[正式容器创建与重命名提交审计](54-container-create-rename-commit-audit.md)接续，并复用单调 edit revision、v1 投影、唯一保存控制器提交和内存基线重建。下一步依次推进锁定/折叠、外观和布局；桌面文件操作继续独立审批，插件小组件只能在容器与布局合同稳定后接入 LPWP 沙箱宿主。
