...
PR #69 CI `30790480972` 已一次通过完整门禁，随后合入 `main` / `0d793f6`。下一步仍应优先完成 Issue #23 的首次整理/引用与移动语义验证，以及 #19–#20 人工和硬件矩阵。只有生产合同和 ADR 获批后，才能新增只读 Desktop Catalog 适配器，并把匿名示例替换为真实但最小化的桌面引用状态。
