...
当前已完成基础解决方案、Core、测试、PR CI 定义和 P0-01a/P0-01b 只读发现探针。后续按以下顺序继续：

1. 确定许可证和支持矩阵；
2. 继续执行稳定身份、Shell 通知、DesktopHost、DPI 和文件安全等 Phase 0 Spike；
3. 记录 ADR-0001 最终结论；
4. 建立最小 DesktopHost 垂直切片；
5. 加入 Nightly Windows 实机矩阵；
6. 完成交互轨原型和可用性验证；
7. DoR 满足后进入 MVP。

现有 CI 已执行真实 restore/build/test/format/依赖漏洞门禁；首次推送后仍需确认 GitHub Actions 远程运行结果。
