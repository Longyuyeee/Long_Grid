...
1. **功能结果优先。** 当前工作必须直接推进核心工程、核心用户旅程或已排队的功能广度，不能用连续探针和文档代替产品进度。
2. **先定义用户看到的结果，再选择实现。**
3. **风险随功能收敛。** 只验证本轮功能真正触及的高风险假设；没有真实回归、数据损坏风险或发布必要性时，不新增相邻安全阶段。
4. **文件系统副作用默认关闭，必须预览、确认和记录。**
5. **不以 Explorer 注入、内部 XAML、`Progman/WorkerW` 或驱动作为核心依赖。**
6. **一次 PR 只交付一个可解释的功能垂直切片；纯探针只允许关闭当前功能阻断。**
7. **测试、文档和迁移属于功能本身，不是收尾工作。**
8. **实验功能与核心路径隔离，失败时自动回退。**
9. **Long Grid 与 Long助手的共享行为先改协议，再改两端实现。**
10. **没有证据的“应该可以”不能升级为架构结论。**
11. **不因为功能优先降低文件安全、隐私、恢复和兼容门槛。**
...
- 每个 PR 必须交付一个用户可演示结果；“新增脚本、合同、适配器或文档”只有在它直接关闭当前功能阻断时才计入产品进度。
- 任一时刻只允许一个主功能执行项。外部环境、签名、人工矩阵和发布审批作为并行门禁维护，除非它们已具备执行条件，否则不能把“等待”写成主开发项。
- 主功能因外部环境无法取得可见证据时，允许完成安全边界内的工程闭环并标记 `ProductEvidencePending`，然后按统一计划进入下一功能；不得继续增加同一阻断的邻接探针。
- 安全缺陷只有在真实复现、可能损坏用户数据/系统状态、破坏恢复能力或阻断当前发布时，才允许中断主功能队列；修正后立即返回原功能项。
