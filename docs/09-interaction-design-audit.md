...
下一增量的“更多”菜单沿用相同有限 activation HWND，不模拟 Explorer 菜单。菜单项必须在打开时就根据锁定、只读、保存失败和目标是否仍存在决定 Enabled；Esc、失焦、系统表面事件或投影替换立即关闭。删除项只打开默认取消的确认流程，并明确“只删除 Long方格配置，不删除真实文件”。
