...
项目负责人随后明确三项 Core：桌面空白处右键创建并管理盒子、盒子绑定真实文件夹、任务栏美化。交互优先级以这三项为中心；自动整理、Tab、Peek、工作空间、小组件和插件均为扩展。桌面右键是盒子创建的主要入口，控制中心和快捷键是可发现替代路径；单文件夹绑定是盒子基础类型，不再延期为高级 Portal；任务栏入口固定在“个性化 → 任务栏”，而不是隐藏在实验功能深层。
...
- 容器属性先展示名称、颜色、布局。
- 规则、条件显示、实例权限等进入高级区域。
- 用户不启用插件或小组件时，不出现相关干扰；任务栏美化作为 Core 始终可在个性化页发现，默认选中“系统默认”。
...
| 个性化 | Long Grid 主题、盒子预设、任务栏美化与恢复 | 是 |
| 小组件 | Widget 目录、实例和权限 | V2 |
| 工作空间 | 应用捕获、启动和状态 | V2 |
| 设置 | 启动、快捷键、隐私、诊断和兼容状态 | 是 |

任务栏美化放在 `个性化 → 任务栏`，与盒子主题并列为 Core。开发期适配器仍用 Feature Flag 和 Windows build 白名单控制风险，但产品信息架构不得把它降级成难以发现的附属实验。
...
2026-08-24 PF-004A 实现增量：正式 DesktopHost 标题现以两行显示折叠方向与名称，以及“项目总数 · 安全引用 · 锁定状态 · 折叠状态”；UIA 使用同一 presentation 输出中文名称和保留“只读方格”边界的机器状态。当前右上角有限入口仍只表示进入 Explicit 交互，不标作更多菜单；直接折叠/锁定属于 PF-004B。
...
- 为纯键盘和辅助技术提供可发现的有限入口：“选择前 256 项”“选择此方格前 256 项”和“清除选择”；不得把 Ctrl/Shift 作为唯一入口；
- 跨容器多选只允许共同动作；
- 批量操作前显示项目数；
- 选择为空时可见状态和 UIA `ItemStatus` 必须显式归零；一次用户动作只播报一次最终数量，程序化逐项选择不得造成最多 256 次重复听读；
- 紧凑宽度下操作栏允许纵向重排，但可见顺序、Tab 顺序和屏幕阅读器顺序必须一致；
- 部分失败后保留失败项选中，便于重试。
...
四层复合探针确认输入重开必须位于最终四层复读之后。补偿也不能边恢复边开放或边恢复边依赖旧 Bounds 验证 UIA；必须先完成全部逆序 Restore，再统一验证。任一层补偿、恢复复读或输入重开失败时，保持交互门关闭并隐藏整个受影响宿主；不得只隐藏失败容器而留下同一 HWND 的透明命中区域。
...
TASKBAR-R2B1-A2 先落实最小正式形态：并列显示“系统默认 / 通透”卡片和一个实时可用性说明。未认证 Build、第三方冲突、只读探测失败或缺少原生适配器时，两个操作必须同时失败关闭并解释用户可理解的原因；不得展示一个可点击但实际无效的按钮。恢复按钮只有存在可验证恢复事务时才开放。该界面准入不等于预设效果完成，完整临时应用、15 秒确认和恢复仍由 R2B1-B/R3 交付。
...
P0-04/P0-05b1 将容器暴露为 UIA List、项目暴露为 ListItem，并让鼠标、键盘与 UIA SelectionItem/Invoke 共享同一选择和调用状态。初次展示不激活，用户点击或辅助技术显式聚焦后才允许获得键盘焦点；永久 `NOACTIVATE` 不满足键盘和屏幕阅读器入口。选中高亮与键盘焦点框必须是两个可区分状态，且均使用系统颜色提供非品牌化 fallback。

该切片目前只使用内存演示项目。人工 Narrator、键鼠、触控、拖放、Win+D/任务视图和真实桌面内容尚未通过，不能因为 UIA Pattern 自动 smoke 成功就宣称无障碍体验完成。

### 16.5 正式批量选择人工验收

正式 `LongGrid.App` 批量选择除结构自动化外，还必须分别执行纯键盘、Narrator、高对比度、200% 文本缩放和小于 760 DIP 紧凑布局。Narrator 场景人工听读单次最终数量与清零播报；视觉场景人工检查焦点、选择、禁用、裁切和重排。AutomationId、Polite live region 或源码断点存在只构成预检，不构成人工 Pass；固定 BSA-01–BSA-05 场景与恢复纪律见[批量选择无障碍人工矩阵运行手册](manual-testing/batch-selection-accessibility-runbook.md)。

## 17. 视觉与动效约束

- 用户可见名称统一为“Long方格”，需要双语时使用“Long方格 · Long Grid”。代码标识继续使用 `LongGrid`。
- 整体风格为现代、扁平、简约且具有精致华丽感；使用清晰网格、低噪声表面、克制圆角、轻量深度和有限品牌强调。
- 渐变、辉光和材质只用于关键品牌时刻或主操作，不覆盖所有卡片和容器；材质不可用时必须有纯色 fallback。
- 桌面容器默认低对比度，不与壁纸争夺注意力。
- 编辑模式用清晰轮廓提高结构感。
- Hover/Pressed 建议 80–140 ms，常规状态动画 120–200 ms，面板进入退出 180–280 ms，布局重排不超过 240 ms。
- 动画必须可中断、可反向并合并过时状态；60 Hz 下以约 16.7 ms 帧预算为基础目标。
- 拖放跟手，不对指针路径做延迟平滑。
- Peek 采用统一进入/退出动效，不逐容器随机延迟。
- 减少动画开启时取消位移和缩放，只保留必要淡入淡出。
- 节能、远程会话、高负载或性能不足时自动降低材质和动效，不影响最终状态、输入门和恢复。
- 颜色不能是状态的唯一表达。

应用图标采用“L + 方格”的单一几何符号：推荐以 3×3 网格为基础，让左侧竖列和底部横列共同形成 L，其余少量方格表达可整理单元。禁止把普通字母简单叠在九宫格上，也禁止 3D 透视、细碎纹理和小尺寸文字。16 px 仍须能辨认 L；完整资产和一键启动/打包要求见[视觉品牌、动效与交付要求](14-visual-branding-and-delivery-requirements.md)。

## 18. MVP 交互优先级

### 必须完成

- 桌面空白处右键创建盒子，并与控制中心/快捷键共用创建结果；
- 单文件夹绑定盒子的绑定、解绑、刷新、打开、离线和权限状态；
- 任务栏预设、临时应用、确认、自动回退和恢复系统默认；
- 首次扫描、双路径起点和建议预览；
- 桌面创建/编辑/拖放；
- 引用与真实移动的动作徽标；
- 容器空、加载、离线、错误状态；
...
## 22. 2026-08-11 正式批量改归属增量

“已加入的引用”列表同时支持批量移除与批量改归属。批量改归属只接受同一源方格内 1..256 个已解析引用；跨方格混选、同一源/目标、锁定方格或超过上限时目标选择器和提交按钮不得形成可提交路径。按钮应显示当前数量，确认框必须默认取消，并明确整批只改变 Long方格配置，不移动、删除或重命名桌面文件。

接受后，状态区一次播报数量、原子结果和可整体撤销一次；撤销成功应明确恢复整批归属。该交互吸收竞品“快速重分组”的效率，但不得隐藏引用与真实文件操作的区别，也不得把跨源复合动作伪装为一次简单归属调整。

## 23. 2026-08-24 桌面标题栏就近命令增量

对标 iTop/Fences 的“方格上直接管理”效率，Long方格先实现三个固定顺序的有限按钮：锁定、折叠、进入交互（视觉从左到右），UIA 导航顺序与命令面内部顺序一致。目标为 32 DIP，并随显示器 DPI 换算；名称必须包含当前动作和方格名，例如“折叠 工作”“解锁 工作”，不能只读出无语义图标。

状态规则：锁定后进入布局交互和折叠均禁用，解锁保持可用；进入 Explicit 选择事务后整个标题命令面暂时撤销，事务退出后恢复。按钮点击不能先乐观改变原生标题再等待保存，而应由唯一 App 状态发布新投影；保存失败恢复原状态并保留可重试提示，避免视觉与磁盘状态分裂。

PF-004C 已按该规范把“更多”接入同一有限 activation HWND，不模拟 Explorer 菜单。重命名、外观和方格列表排序导航到唯一控制中心；规则、Portal/Tab 和删除以禁用后续项显示。菜单项在打开时就根据锁定、只读、保存失败和目标是否仍存在决定 Enabled；Esc、失焦、系统表面事件或投影替换立即关闭。PF-004D 再启用删除项并只打开默认取消的确认流程，明确“只删除 Long方格配置，不删除真实文件”。

PF-004E 已把标题交互冻结为有限且可预测的策略：显示仅允许始终、悬停、隐藏；双击仅允许折叠/展开或无动作。桌面宿主只命中标题文字区，其余区域保持穿透，右侧四个命令仍由独立 activation HWND 拥有。隐藏标题不隐藏 UIA 名称或控制中心入口；锁定方格拒绝双击折叠。控制中心外观区同时展示颜色、透明度、标题显示和标题双击，四者一次原子保存并可由唯一最近撤销恢复。

PF-005A 将项目行从圆点文本改为“系统类型图标 + 可见名称 + 必要状态前缀”。File、Folder、Shortcut、URL 使用 Windows stock icon；离线、类型变化、身份歧义和不支持统一使用 Warning 回退并显示短状态，不使用破损图片占位。UIA 名称格式固定为“可见名称；类型；有限状态”，不得附加 canonical target 或完整路径。图标保持 20 DIP，在 100%～400% DPI 由系统缩放；PF-005B 的真实缩略图必须沿用相同占位尺寸，避免加载完成时项目行跳动。
