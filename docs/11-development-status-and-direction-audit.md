...
- Issue #24 已建立正式配置存储、工作区/Catalog 转换、reducer、连续保存、有限产品会话、用户/公共桌面第一层只读 Catalog，以及未解析引用匿名审查：默认保留、显式重选和移除确认均经 Catalog generation + edit revision + 状态/锁定/候选唯一性门禁，只生成 reducer 预演。文件操作与普通保存仍为零；Shell 虚拟项、干净会话真实 93-ID UIA、首次普通入队、自动保留/容量策略和完整关闭矩阵仍未完成，真实卷证据继续 Pending；
...
| 配置持久化 | 正式 v1、状态/Catalog 转换、reducer、连续保存、App 保存 UIA、有限产品会话、用户/公共桌面只读 Catalog、未解析引用匿名预演及 generation+revision 门禁、Infrastructure 原子存储/恢复/证据管理、完整单实例；Spike 另覆盖 Shell、强杀、迁移、只读/磁盘满/权限恢复 | E2-E3 / gated dry-run + App session pass | 仍缺首次普通入队、Shell 虚拟项、干净会话真实 93-ID UIA、获批保留策略、完整关闭竞态、真实卷、正式 v2 与跨进程公平性 |
