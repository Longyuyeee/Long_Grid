...
| W4（会话就绪） | 必要安全、隔离与持久化收口（#21、#22、#24） | 正式 Core v1 配置合同、UI 无关产品状态与 Catalog 的双向有限转换、未解析引用无损保留、Infrastructure 存储、有限保存/显式重试工作流及 I24-01/I24-02 专用环境入口已建立；演示 UI 保持零入队，产品 reducer、连续编辑保存状态机、保存 UIA 和真实卷证据仍 Pending | 不误移动，安全边界明确，生产合同可迁移；真实产品接线列入首片验收 |
