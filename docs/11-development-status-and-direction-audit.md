...
| 配置持久化 | 版本化 JSON、原子替换/备份/安全模式、四检查点及 ACL 生效期间强杀、跨进程单写租约、有界退避、具备入队快照/有界排空的 latest-wins 保存协调、确定性迁移回滚、只读/磁盘满/权限恢复 | E2 / Conditional Pass | 1,000 次四检查点强杀、断电、真实卷空间耗尽/只读、替换内部失败、真实应用关闭接线、跨进程公平性、完整单实例激活和正式 schema 矩阵 |
