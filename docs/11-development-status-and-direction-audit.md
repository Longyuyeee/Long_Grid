...
审计基线：`main` / `dc7f7f5`，加本轮受限 Low Integrity 边界切片
...
| 代码 | 当前分支有 17 个 Core、12 个测试和 47 个探针 C# 源文件 | 以风险验证为主 |
...
| 图标/缩略图 | 异步加载、取消、句柄闭环；同权限 worker 回收、硬超时、有界协议故障恢复、父进程 PID/Job Object 双重退出清理、连续超时退避、有界 BGRA32 像素 IPC、受限 Low Integrity 读/写边界和合成 500 项预算 | E2 / Conditional Pass | 实际 worker 降权、broker、共享内存/渲染集成、真实 provider/支持矩阵与最终预算批准 |
...
PR #18、#25–#36 已进入 `main`，当前审计基线为合并 PR #36 后的 `dc7f7f5`。覆盖率、配置、文件安全、缩略图 worker 和依赖漏洞门禁已在主干 CI `30689450777` 全部通过。PR #2–#17 已关闭；删除远端分支前，对历史功能分支逐一确认其提交已进入主干。远端现只保留 `main`。
...
### 4.6 低权限边界已有首轮证据，但不是正式 worker 隔离

PR #35 已把缩略图 worker 从“只返回状态/尺寸”推进到协议 v2 的有界 BGRA32 像素负载。本轮又创建 `DISABLE_MAX_PRIVILEGE` 受限令牌，将 Mandatory Integrity Level 设置并查询为 Low（RID `0x1000`），再用模拟上下文证明自有 BMP 可读、向默认中完整性沙箱的 write-up 被阻断，父进程写控制组仍成功。500/500 worker 矩阵、硬超时、恢复、Job Object、像素协议与预算同时通过。

这只关闭了“低权限访问边界完全没有运行证据”，不能写成正式 worker 已降权。下一步仍不能用进程内模拟替代两类前置决定：

- 必须比较 AppContainer 与受限 Low Integrity token，并定义文件 broker/句柄传递与 Capability；随后用所选令牌启动实际 worker，复测管道、Job Object、硬超时和退出清理；
...
- [x] PR #18 经审核后合入 `main`；
- [x] PR #2–#18、#25–#36 已合并或关闭，远端无长期串联草稿分支；
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#36 已合入；PR #2–#17 关闭；旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
