...
审计基线：`main` / `ec8e79f`（PR #71 已合入）+ 匿名项目与拖放语义原型分支
...
| 正式 App UI 壳层 | WinUI 导航、Design Token、内存态主题、AutomationId/访问键、760 DIP 响应式流、DPI 感知窗口、Core 只读状态、首次整理模式选择/预览、匿名容器、三个匿名引用、拖放语义和两步撤销的真实 UIA 冒烟 | E2 / Conditional Pass | 5 人测试、真实拖放与恢复差异原型、负责人决策、经批准的真实只读数据适配器、Narrator/高对比/系统文本缩放/多 DPI 视觉矩阵 |
...
- 真实拖入容器、规则预览和恢复差异的连贯低保真原型；首次整理、匿名添加引用、拖放语义和两步撤销已有自动化原型；
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#61、#63–#71 已合入；PR #2–#17 关闭；旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、当前切片前无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
| W1（进行中） | 产品与体验决策（#23） | 首次整理、匿名容器、三个匿名引用、拖放语义和两步撤销原型已完成；待恢复差异原型、5 人测试、许可证、支持矩阵、首版模式 | 负责人签字确认范围与预算，并限定后续技术矩阵 |
