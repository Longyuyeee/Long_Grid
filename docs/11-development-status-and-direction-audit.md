...
审计基线：`main` / `3f1f96a`（PR #92 已合入）+ 证据生命周期基础增量分支
...
- Issue #24 已建立正式配置存储、工作区/Catalog 转换、reducer、连续保存、有限产品会话，以及用户/公共桌面第一层只读 Catalog：双来源完整性决定权威性，generation/latest-wins 防止旧刷新覆盖，Partial 不参与 Missing 判断，Core 只报告 ConnectedReadOnly。匿名 UI、文件操作与普通保存仍为零；未解析引用 UI、generation+revision 编辑门禁、Shell 虚拟项、干净会话真实 85-ID UIA、首次普通入队、自动保留/容量策略和完整关闭矩阵仍未完成，真实卷证据继续 Pending；
...
| 配置持久化 | 正式 v1、状态/Catalog 转换、reducer、连续保存、App 保存 UIA、有限产品会话、用户/公共桌面只读 Catalog generation/权威门禁、Infrastructure 原子存储/恢复/证据管理、完整单实例；Spike 另覆盖 Shell、强杀、迁移、只读/磁盘满/权限恢复 | E2-E3 / read-only catalog + App session pass | 仍缺未解析引用 UI、generation+revision 编辑门禁、Shell 虚拟项、干净会话真实 85-ID UIA、普通入队、获批保留策略、完整关闭竞态、真实卷、正式 v2 与跨进程公平性 |
...
| W4（会话就绪） | 必要安全、隔离与持久化收口（#21、#22、#24） | 正式 v1、状态/Catalog 转换、未解析保留、reducer、连续保存、App UIA、有限会话、物理只读 Catalog generation/权威门禁、Infrastructure 存储及专用环境入口已建立；匿名 UI 零入队，未解析 UI/编辑组合门禁、Shell、干净会话 UIA、普通入队、完整关闭矩阵和真实卷仍 Pending | 不误移动，安全边界明确，生产合同可迁移；真实产品接线列入首片验收 |
