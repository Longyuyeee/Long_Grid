...
Long Grid 已经越过“空仓库”和“只写方案”的阶段，形成了可复现的 .NET 工程基线、纯 Core 决策逻辑、真实 Win32/Shell/DirectComposition/UI Automation 探针以及自动化测试。桌面发现、稳定身份、Shell 变化对账、图像提取、DesktopHost 窗口模型、显示拓扑、布局恢复、事务补偿、首个可交互宿主切片和配置持久化均有代码与报告证据。PR #18 已合入 `main`；配置探针通过了 20 类场景、1,000 次重复写入、四检查点共 1,000 次真实进程强杀及 ACL 生效期间 10 次强杀。
...
| 解决方案 | `main` 有 1 个 Core、1 个测试和 8 个探针项目 | 已建立，但不是产品分层 |
...
| 图标/缩略图 | 异步加载、取消、句柄闭环；同权限 worker 回收、硬超时、64 KiB 流级上限、协议故障恢复和合成 500 项预算 | E2 / Conditional Pass | 低权限 token、像素 IPC、父进程退出/孤儿回收、真实 provider/支持矩阵与最终预算批准 |
...
PR #18 已以合并提交 `7e3ac8a` 进入 `main`，合入后的主干 CI 全部通过。PR #2–#17 已关闭；删除远端分支前，对全部 18 条功能分支逐一确认其为 `origin/main` 的祖先且相对主干独立提交数为 0。远端现只保留 `main`。
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18 合入为 `7e3ac8a`；PR #2–#17 关闭；18 条旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
