...
审计基线：`main` / `fa40012`（PR #79 已合入）+ Issue #23 范围批准分支
...
- Issue #21–#22 已完成关闭就绪对表；D23 批准后，#21 的托管移动专用矩阵移出首发阻断项，#22 以 Windows 11 x64、隔离 Worker 安全分类和类型图标回退收口。现有自动探针继续作为回归门禁，未执行项保留后续里程碑而不提升为 Pass；
...
2. 按 Windows 11 x64、安全引用和类型图标回退范围收口 #21–#22，不再机会主义扩展已经充分举证的探针族；
...
| GitHub | `main` 已合入至 PR #79；当前 Issue #23 范围批准使用短生命周期分支；Phase 0 Exit 里程碑跟踪 #19–#24 六项工作 | 治理基线已闭环；当前切片仍须 PR/CI |
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#61、#63–#79 已合入；PR #2–#17 关闭；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、保护规则要求严格 `build-test` |
