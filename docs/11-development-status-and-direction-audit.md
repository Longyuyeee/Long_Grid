...
- 干净会话入口统一编排 139-ID UIA 与单实例验证；本轮只完成源码合同和 `ValidateOnly`，live 证据保持 Pending，脚本继续拒绝终止外来进程；
...
- Issue #24 已建立正式配置存储、有限产品会话、用户/公共桌面只读 Catalog、1..256 原子批量添加未分组引用、同方格原子批量移除、同源方格原子批量改归属、确认删除正式方格配置，以及最近一次布局恢复、批量加入、批量移除、批量改归属或方格删除的一次撤销；统一入口只投影恰好一个有效令牌，冲突状态默认关闭，原有上下文入口继续保留。批量列表还具备键盘可达的有限选择与显式清除、空选择状态复位、单次 live-region 播报和紧凑布局纵向重排，其 BSA 专项人工入口已经就绪但真实结果仍 Pending。同时具备有限产品编辑、正式工作区视图、生产级当前显示拓扑、v2 保存时拓扑、配置级恢复确认和同会话一次性撤销。各类撤销令牌绑定操作 ID、变更 revision 与配置前后指纹，其他成功编辑使其失效。它们都只进入统一保存控制器，所有桌面文件与真实窗口操作为零。Shell 虚拟项、真实窗口提交、干净会话真实 139-ID UIA、自动保留/容量策略和完整关闭矩阵仍未完成，真实卷证据继续 Pending；
...
| 配置持久化 | 正式 v2、v1 迁移、状态/Catalog 转换、reducer、连续保存、App 保存 UIA、有限产品会话、用户/公共桌面只读 Catalog、原子批量添加/同方格批量移除/同源方格批量改归属引用、键盘可达的有限批量选择操作栏、方格 CRUD、各动作一次撤销与统一最近编辑入口、保存时/当前拓扑双门禁、配置级恢复确认与一次性撤销、Infrastructure 原子存储/恢复/证据管理、完整单实例 | E2-E3 / formal config-only recovery and undo | 仍缺真实窗口提交、Shell 虚拟项、干净会话真实 139-ID UIA、获批保留策略、完整关闭竞态、真实卷与跨进程公平性 |
