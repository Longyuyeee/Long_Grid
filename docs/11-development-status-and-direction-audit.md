...
- Issue #24 已建立正式配置存储、有限产品会话、用户/公共桌面只读 Catalog、有限产品编辑、正式工作区视图、布局恢复只读预览与生产级当前显示拓扑适配器：CCD/Monitor 数量、强身份、Bounds、target、rotation、WorkArea 全部成立才权威，generation/latest-wins 和关闭排空已接入 App；v1 缺少保存时拓扑时保持 SavedTopologyMissing，降级样本保持 Awaiting，不冒充 Automatic。所有桌面文件与真实窗口操作为零。Shell 虚拟项、版本化保存时拓扑、干净会话真实 116-ID UIA、自动保留/容量策略和完整关闭矩阵仍未完成，真实卷证据继续 Pending；
...
| 配置持久化 | 正式 v1、状态/Catalog 转换、reducer、连续保存、App 保存 UIA、有限产品会话、用户/公共桌面只读 Catalog、引用编辑、脱敏正式工作区、共享 revision 的有限容器编辑、布局恢复双拓扑门禁只读预览、生产级只读当前拓扑、Infrastructure 原子存储/恢复/证据管理、完整单实例 | E2-E3 / formal read surface and bounded container edits | 仍缺版本化保存时拓扑、真实窗口提交、删除/撤销、Shell 虚拟项、干净会话真实 116-ID UIA、获批保留策略、完整关闭竞态、真实卷、正式 v2 与跨进程公平性 |
