...
审计基线：`main` / `0d793f6`（PR #69 已合入）+ 首次整理模式原型分支
...
| 正式 App UI 壳层 | WinUI 导航、Design Token、内存态主题、AutomationId/访问键、760 DIP 响应式流、DPI 感知窗口、Core 只读状态、首次整理模式选择/预览与真实 UIA 冒烟 | E2 / Conditional Pass | 5 人测试、负责人决策、经批准的真实只读数据适配器、Narrator/高对比/系统文本缩放/多 DPI 视觉矩阵 |
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#61、#63–#69 已合入；PR #2–#17 关闭；旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、当前切片前无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
| W1（进行中） | 产品与体验决策（#23） | 首次整理模式可点击原型已完成；待 5 人测试、许可证、支持矩阵、首版模式 | 负责人签字确认范围与预算，并限定后续技术矩阵 |
