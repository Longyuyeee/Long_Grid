...
审计基线：`main` / `7721384`（PR #68 已合入）+ Core 只读运行状态合同分支
...
| GitHub | `main` 已合入至 PR #68；当前 Core 状态接线使用短生命周期分支；Phase 0 Exit 里程碑跟踪 #19–#24 六项工作 | 治理基线已闭环；当前切片仍须 PR/CI |
...
| 正式 App UI 壳层 | WinUI 导航、Design Token、内存态主题、AutomationId/访问键、760 DIP 响应式流、DPI 感知窗口、Core 只读能力状态快照与真实 UIA 冒烟 | E2 / Conditional Pass | 经批准的真实只读数据适配器、Narrator/高对比/系统文本缩放/多 DPI 视觉矩阵和 5 人测试 |
...
PR #18、#25–#61、#63–#68 已进入 `main`，当前主干基线为合并 PR #68 后的 `7721384`。响应式 UI、覆盖率、配置、文件安全、零 Capability AppContainer 真实缩略图 worker、UI 自动化合同和依赖漏洞门禁已在 PR #68 最终 CI `30788800640` 第二次运行全部通过；第一次运行在执行仓库代码前卡于 Setup .NET，取消后同一提交重跑成功。PR #2–#17 已关闭；当前新切片建立前远端仅保留 `main`。
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#61、#63–#68 已合入；PR #2–#17 关闭；旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、当前切片前无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
