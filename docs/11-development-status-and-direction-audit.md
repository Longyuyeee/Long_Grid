...
- Issue #24 已建立正式配置存储、latest-wins/App 关闭排空、完整单实例、不携带 Document/路径/原始错误的恢复状态 UI，以及经二次确认的恢复动作、当前 v1 本地 JSON 受限导入/导出、匿名证据清单、单项原始证据显式导出、观察条数/容量/最早时间和用户确认的单项永久清理。清理具备默认取消、清理前导出提示、来源与快照复检、有界写租约和有限结果；未批准阈值前不自动删除。正式 v1 是首个产品 schema，迁移等待真实 v2 字段准入；尚未完成自动保留/容量策略、真实产品状态入队或关闭排空竞态矩阵，真实卷证据继续 Pending；
...
| 配置持久化 | 正式 v1 schema、Infrastructure 原子发布/备份/安全模式/证据保护/有界写租约、latest-wins、App 5 秒关闭排空、完整单实例、恢复状态 UI、经确认的恢复动作、受限导入/导出、匿名证据清单、单项原始证据导出、观察生命周期统计和单项永久清理；Spike 另覆盖强杀、迁移、只读/磁盘满/权限恢复 | E3 / Production lifecycle slice pass | 正式迁移等待真实 v2 字段准入；仍缺获批的自动保留/容量策略、真实状态入队、关闭竞态矩阵、真实卷耗尽/只读、替换内部失败与跨进程公平性 |
...
| W4（会话就绪） | 必要安全、隔离与持久化收口（#21、#22、#24） | 正式 Core v1 配置合同、UI 无关产品工作区状态/Catalog 身份投影、Infrastructure 存储、有限保存/显式重试工作流及 I24-01/I24-02 专用环境入口已建立；演示 UI 保持零入队，配置到 Catalog 的 missing/unresolved 恢复、产品 reducer、保存状态 UI 和真实卷证据仍 Pending | 不误移动，安全边界明确，生产合同可迁移；真实产品接线列入首片验收 |
