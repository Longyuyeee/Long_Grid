...
| 图标/缩略图 | 异步加载、取消、句柄闭环；同权限 worker 回收、硬超时、64 KiB 流级上限、协议故障恢复、父进程退出清理、连续超时退避和合成 500 项预算 | E2 / Conditional Pass | 低权限 token、像素 IPC、Job Object 生产兜底、真实 provider/支持矩阵与最终预算批准 |
...
PR #18、#25–#31 已进入 `main`，当前审计基线为合并 PR #31 后的 `ec47216`，新增覆盖率、文件安全和缩略图 worker 门禁已在远端 CI 通过。PR #2–#17 已关闭；删除远端分支前，对历史功能分支逐一确认其提交已进入主干。远端现只保留 `main`。
...
- [x] PR #18 经审核后合入 `main`；
- [x] PR #2–#18、#25–#31 已合并或关闭，远端无长期串联草稿分支；
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#31 已合入；PR #2–#17 关闭；旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
