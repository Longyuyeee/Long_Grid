...
审计基线：`main` / `853a606`（PR #73 已合入）+ Issue #24 正式配置合同分支
...
| W4（合同进行中） | 必要安全、隔离与持久化收口（#21、#22、#24） | 正式 Core v1 配置合同已建立；待批准范围内的文件/Provider/真实卷证据和 Infrastructure 适配器 | 不误移动，安全边界明确，生产合同可迁移；集成接线列入首片验收 |
