...
但这些成果大多仍是 **Conditional Pass 的风险探针**，不是可发布产品能力。仓库现已建立开发期只读 `LongGrid.App` UI Shell、Design Token、品牌 RC1、一键启动链，以及正式 `LongGrid.Infrastructure` 配置存储和 App 关闭排空接线；UI 切片仍只显示匿名示例数据，不枚举真实桌面、不连接 DesktopHost、不执行文件操作，也没有配置保存入队入口或安装流程，因此证据等级仍是 E2/E3 开发集成，而不是 E4 产品切片。`LongGrid.DesktopHost`、安装包和端到端产品流程仍不存在。真实键鼠/触控/拖放、Narrator、Win+D、全屏、Explorer 重启、动态显示硬件矩阵、文件移动撤销、真实卷故障、真实 Provider 性能矩阵和可用性测试尚未关闭。
...
- Issue #24 已建立正式配置存储、latest-wins/App 关闭排空、完整单实例、不携带 Document/路径/原始错误的恢复状态 UI，以及经二次确认的已验证备份接受、SafeMode 空白安全重置和主备损坏证据归档。尚未完成外部导入、真实产品状态入队或关闭排空竞态矩阵，真实卷证据继续 Pending；
...
| 测试 | 当前全量 152 项测试通过 | Core、正式配置存储/协调器、有限恢复状态、备份接受与 SafeMode 重置回归基线有效 |
...
| GitHub | `main` 已合入至 PR #88；#21–#22 已按批准范围关闭，#23 保持打开并等待 P1–P5 真实结果；Phase 0 Exit 继续跟踪 #19、#20、#23、#24 | 治理基线已闭环；后续切片仍须 PR/CI |
| 主干保护 | `main` 要求严格的 `build-test`，对管理员生效，禁止强推和删除 | 已建立最小可信门禁 |
| 许可证 | GitHub 未识别许可证，仓库根目录无 LICENSE | 阻断公开分发与外部贡献 |
| 覆盖率 | 152 项测试通过；本分支单次采集行覆盖率 91.73%（1796/1958）、分支覆盖率 80.48%（474/589），主线 CI 门禁分别为 90%/75% | Phase 1 关键 Core 分支目标 ≥80%，当前先跟踪趋势、不用总百分比替代风险测试 |
...
| 配置持久化 | 正式 v1 schema、Infrastructure 原子发布/备份/安全模式/证据保护/有界写租约、latest-wins、App 5 秒关闭排空、完整单实例、恢复状态 UI、经确认的备份接受、SafeMode 空白重置与主备证据归档；Spike 另覆盖强杀、迁移、只读/磁盘满/权限恢复 | E3 / Production lifecycle slice pass | 仍缺外部导入/证据清理、真实状态入队、关闭竞态矩阵、真实卷耗尽/只读、替换内部失败与跨进程公平性 |
...
PR #18、#25–#61、#63–#88 已进入 `main`，当前主干基线为合并 PR #88 后的 `0f3959b`。Core 测试、覆盖率、配置、文件安全、缩略图 Worker、UI 自动化合同和依赖漏洞门禁已在主线 CI `30893837178` 通过。PR #2–#17 已关闭；后续继续使用从最新 `main` 建立的短生命周期分支。
