...
| 配置持久化 | 版本化 JSON、同目录暂存、落盘校验、原子替换、备份恢复、安全模式、未知字段保留、四检查点子进程强杀、跨进程单写租约、确定性示例迁移回滚、四类只读文件恢复、受控磁盘满注入和最小目录 ACL 拒绝恢复 | E2 / Conditional Pass | 1,000 次强杀、断电、真实卷空间耗尽、替换内部失败、只读目录/扩展权限、完整单实例激活和正式 schema 矩阵 |
