...
- Issue #24 已建立正式配置存储、latest-wins/App 关闭排空、完整单实例，以及不携带 Document/路径/原始错误的只读恢复状态 UI。尚未完成显式修复、真实产品状态入队或关闭排空竞态矩阵，真实卷证据继续 Pending；
...
| 测试 | 当前全量 137 项测试通过 | Core、正式配置存储/协调器与有限恢复状态回归基线有效 |
...
| GitHub | `main` 已合入至 PR #86；#21–#22 已按批准范围关闭，#23 保持打开并等待 P1–P5 真实结果；Phase 0 Exit 继续跟踪 #19、#20、#23、#24 | 治理基线已闭环；后续切片仍须 PR/CI |
| 主干保护 | `main` 要求严格的 `build-test`，对管理员生效，禁止强推和删除 | 已建立最小可信门禁 |
| 许可证 | GitHub 未识别许可证，仓库根目录无 LICENSE | 阻断公开分发与外部贡献 |
| 覆盖率 | 137 项测试通过；本分支行覆盖率 91.81%（3294/3588）、分支覆盖率 80.00%（888/1110），主线 CI 门禁分别为 90%/75% | Phase 1 关键 Core 分支目标 ≥80%，当前先跟踪趋势、不用总百分比替代风险测试 |
...
| 配置持久化 | 正式 v1 schema、Infrastructure 原子发布/备份/安全模式/证据保护/有界写租约、latest-wins、App 5 秒关闭排空、完整单实例及只读恢复状态 UI；Spike 另覆盖强杀、迁移、只读/磁盘满/权限恢复 | E3 / Production lifecycle slice pass | 仍缺显式修复、真实状态入队、关闭竞态矩阵、真实卷耗尽/只读、替换内部失败与跨进程公平性 |
...
PR #18、#25–#61、#63–#86 已进入 `main`，当前主干基线为合并 PR #86 后的 `ea81f21`。Core 测试、覆盖率、配置、文件安全、缩略图 Worker、UI 自动化合同和依赖漏洞门禁已在主线 CI `30888761175` 通过。PR #2–#17 已关闭；后续继续使用从最新 `main` 建立的短生命周期分支。
