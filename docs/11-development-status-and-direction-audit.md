...
审计基线：`codex/p0-interactive-desktop-host-slice` / `22d72ca`
...
1. 把 17 个串联草稿 PR 按顺序集成进 `main`，消除“分支上已完成、主干上不可用”的交付风险；
2. 完成剩余 Phase 0 技术轨和体验轨门禁；
...
| GitHub | PR #1-#17 均为串联草稿 PR，检查通过 | 集成债务为当前 P0 |
...
| 交互切片 | 一个可见 List 容器和三个进程内演示项 | E2 / Conditional Pass | 文件语义、拖放、持久化和用户测试 |
...
PR #1 到 #17 使用“前一个功能分支作为后一个 PR 基线”的串联结构。它保证了每个差异较小，但也意味着任何前序 PR 未合并或被修改，都会向后传播重基线成本。所有 PR 仍是 Draft，因此当前成果不能被视为已经交付到 `main`。

处理要求：

1. 暂停新增功能型堆叠 PR；
2. 从 #1 开始审阅和合并；
3. 每合并一个 PR，就把下一 PR 的 base 调整为 `main`，确认差异只剩该层；
4. 每层重新等待 CI，通过后再继续；
5. #17 集成完成后，从最新 `main` 建立后续分支；
6. 不用一次性巨型 squash 代替逐层证据审查。

### 4.2 “探针已通过”与“产品已实现”边界总体正确，但入口文档有滞后

路线图和 Spike 报告普遍保留了 Conditional Pass，ADR-0001 也仍为 Proposed，这是正确的。需要修正的地方：

- 原始仓库审计仍以“空仓库 / Greenfield”为主结论，只适合作为历史基线；
- README 仍称可见输入门和完整 Fragment 树未关闭，已落后于 P0-05b1/P0-07b2b2b2b4a；
- 开发流程的“仓库落地顺序”仍停留在 P0-01a/P0-01b；
- 路线图需要把 PR 集成作为进入下一阶段前的显式门禁。
...
- PR #1-#17 依次完成审阅和集成；
- 最新 `main` 全量 CI 通过；
- 路线图完成项在 `main` 上均能找到代码、测试和报告；
- 无长期存在的串联草稿分支；
- 后续 PR 从最新 `main` 创建。

在 Gate A 完成前，只允许修正文档、修复 CI/回归或解决合并阻断，不继续扩展功能栈。
...
1. **集成工作包**：清理 PR #1-#17，产出最新绿色 `main`；
...
最终判断：**方向正确，底层风险控制质量较高；当前瓶颈已从“技术未知”转为“主干集成、真实矩阵、体验验证和产品决策”。先关闭这些门禁，再开始生产化 MVP，是风险和交付效率最优的路径。**
