...
PR #18、#25–#37 已进入 `main`，当前审计基线为合并 PR #37 后的 `8e04b5a`。覆盖率、配置、文件安全、缩略图 worker 和依赖漏洞门禁已在主干 CI `30689886557` 全部通过。PR #2–#17 已关闭；删除远端分支前，对历史功能分支逐一确认其提交已进入主干。远端现只保留 `main`。
...
PR #35 已把缩略图 worker 从“只返回状态/尺寸”推进到协议 v2 的有界 BGRA32 像素负载。PR #37 又创建 `DISABLE_MAX_PRIVILEGE` 受限令牌，将 Mandatory Integrity Level 设置并查询为 Low（RID `0x1000`），再用模拟上下文证明自有 BMP 可读、向默认中完整性沙箱的 write-up 被阻断，父进程写控制组仍成功。500/500 worker 矩阵、硬超时、恢复、Job Object、像素协议与预算同时通过。
...
- [x] PR #18 经审核后合入 `main`；
- [x] PR #2–#18、#25–#37 已合并或关闭，远端无长期串联草稿分支；
...
| 顺序 | 工作包 | 交付物 | 退出条件 |
|---|---|---|---|
| W0（完成） | GitHub 治理 | PR #18、#25–#37 已合入；PR #2–#17 关闭；旧远端分支删除；`main` 受保护；Phase 0 Exit 建立 | 主干 CI 通过、无打开 PR、远端只保留 `main`，保护规则要求严格 `build-test` |
