...
| #22 | AppContainer 与访问 broker | 真实 worker 全部为零 Capability AppContainer；协议 v6 对照受控副本与最小路径 ACL；BMP/PNG/GIF/JPEG/TIFF 逐项验证输入可读、同格式策略一致、安全分类和 Profile 清理，正常路径复核随机 SID ACE 已恢复 | Conditional Pass（自动探针）；ACL 只作比较，异常退出 ACE 修复 Pending |
...
| #22 | 真实 Provider、x64/ARM64、Windows 矩阵 | 自有五格式：22621 前四格式 8/8 提取成功、TIFF 2/2 精确 `ERROR_MOD_NOT_FOUND`；26100 10/10 可读但 Shell 全部 `E_ACCESSDENIED`，必须类型图标/缓存回退；只输出固定格式标签/HRESULT | Partial；HEIF、Office/PDF、云/网络、第三方、ARM64 矩阵及 TIFF 模块定位 Pending |
