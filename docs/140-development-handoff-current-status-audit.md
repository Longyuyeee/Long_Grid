...
5. **M4c2a：正式 App 24 小时资源合同**——冻结入口与预算；当前正式 worker 和匿名 revision telemetry 缺口必须显式阻止 Pass；
6. **M4c2b/M4c2c：正式接线与真实执行**——接入受限 worker/遥测后，在支持设备验证 Surface、UIA、目录、worker/Profile、窗口、handle/thread/private bytes 与状态漂移；
7. **外部证据汇合**——按 C1 → C2/C3 → C4 → C5b → C6 完成五人测试、输入/无障碍/系统表面、显示硬件、专用卷和 ADR/Issue 收口；
8. **内部 RC**——许可证、签名、安装/升级/卸载/回滚、正式品牌确认完成后，才允许把内部工程产物提升为可分发候选；
9. **MVP 后扩展**——任务栏外观实验、LongBar、Widget Host、LPWP 插件兼容和广泛窗口视觉能力分别立项、隔离权限与资源预算，不进入核心常驻路径。
