...
- **PR CI / main CI**：PR #197 run `31806277821` 与合并后 main run `31807522227` 均为 921/921；line 90.38%/90.39%，branch 78.82%/78.84%，完整门禁成功；squash 合并 `main@91ff6fdabb931b8583fdb66b6a9c43f5857163d4`；
...
- **远端证据**：PR #199 run `31809749693` 与合并后 main run `31810369292` 均为 922/922，lines 90.51%，branches 78.60%，恢复预检 5/5 与完整门禁通过；squash 合并 `main@a47a19d`；
...
### M4b2：Explorer、显示器与 DesktopHost 生命周期（工程完成）
...
- **远端证据**：PR #201 run `31812773516` 与合并后 main run `31813422201` 均为 923/923，lines 90.68%，branches 78.34%，M4b2 5/5 与完整门禁通过；squash 合并 `main@8d04e2f`；
...
- **远端证据**：PR #203 run `31815841350` 为 924/924，lines 90.81%、branches 78.20%；squash 合并 `main@484e8cc` 后 main run `31816415351` 为 924/924，lines 90.83%、branches 78.22%；两次均通过加速预检、缩略图 worker 隔离门与完整交付集审计；
...
### M4c2b1：正式 App 匿名资源遥测（Engineering Pass）
...
- **远端证据**：PR #207 run `31822678611` 与 squash 合并 `main@5f4a5c2` 后 main run `31823246761` 均为 929/929、lines 90.85%、branches 77.97%；格式、正式 App Build、匿名遥测合同、安全/worker 隔离和 unsigned RC 交付集门全部通过；
...
### M4c2b2：正式受限缩略图 worker（Engineering Pass）
...
- **远端结果**：PR #209 run `31827595353` 为 935/935、lines 90.87%、branches 78.06%；squash 合并 `main@1689446` 后 main run `31828145653` 为 935/935、lines 90.86%、branches 78.04%。两次均通过正式 App build/startup、产品 worker 500 矩阵、依赖门和 800 文件内部 unsigned RC 交付集审计；
- **结论**：M4c2b2 Engineering Pass；`formalThumbnailWorkerIntegrated=true`，但真实 24 小时未执行，`canProduceM4cPass=false`；
- **下一步**：由 M4c2c 从 `main@1689446` 执行和审计真实 24 小时会话。详见 [Stage 147](147-formal-restricted-thumbnail-worker-integration-audit.md)。

### M4c2c-r：真实会话复审门禁（工程就绪，live Pending）

- **审计基线**：`main@5e3e560`；
- **审计发现**：原会话缺少临时 AppContainer Profile 的 `0/0` 结束证明，也没有从原始样本独立复算趋势的工具；
