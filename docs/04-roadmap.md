...
- [x] P0-03a 图标/缩略图异步加载、排队取消与句柄稳定性；P0-03b 受限 Low Integrity 可回收 worker、挂起启动后先加入 Job、显式继承句柄列表、250 ms 硬超时、有界协议/异常退出恢复、父进程 PID/Job Object 双重退出清理、连续超时退避、显式复制匿名映射句柄的有界 BGRA32 IPC 及合成 500 项延迟/CPU/内存/句柄预算（Conditional Pass；AppContainer/受限 token 与文件 broker 决策、正式渲染集成和真实 provider/支持矩阵未关闭）。
...
- [x] PR #2–#18、#25–#42 已收口，相关代码进入 `main`，旧远端功能分支已删除。
