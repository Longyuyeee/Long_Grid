...
- [x] P0-06 版本化配置、同目录暂存、落盘校验、原子替换、备份恢复、安全模式、四检查点子进程强杀、跨进程单写租约、确定性 v1→v2 示例迁移回滚、四类只读文件恢复及受控磁盘满注入（Conditional Pass；1,000 次强杀、断电、真实卷空间耗尽、目录 ACL 和正式 schema 矩阵未关闭）。
