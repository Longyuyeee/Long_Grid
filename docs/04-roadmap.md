...
- [x] P0-06 版本化配置、同目录暂存、落盘校验、原子替换、备份恢复、安全模式、四检查点及 ACL 生效期间子进程强杀、跨进程单写租约、确定性 v1→v2 示例迁移回滚、四类只读文件恢复、受控磁盘满注入、创建/替换 ACL 拒绝恢复、内容写入/父目录继承 ACL 与目录只读属性语义（Conditional Pass；1,000 次四检查点强杀、断电、真实卷空间耗尽/只读和正式 schema 矩阵未关闭）。
