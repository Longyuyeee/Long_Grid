...
- [x] P0-03a 图标/缩略图异步加载、排队取消与句柄稳定性；P0-03b 零 Capability AppContainer 真实 worker、挂起启动后先加入 Job、显式继承句柄列表、32 MiB 受控只读输入、协议 v6 的 `ControlledCopy`/`MinimumPathAcl` 对照、250 ms 硬超时、父进程退出/Profile 清理、连续超时退避、有界共享内存 BGRA32 IPC、未代理读写阻断及合成 500 项预算（Conditional Pass；ACL 方案在 26100 同样安全返回 `E_ACCESSDENIED`，仍以副本为默认实验；正式渲染和 provider/支持矩阵未关闭）。
...
- [x] PR #2–#18、#25–#50 已收口，相关代码进入 `main`，旧远端功能分支已删除。
