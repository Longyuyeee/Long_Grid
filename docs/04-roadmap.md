...
- [x] P0-03a 图标/缩略图异步加载、排队取消与句柄稳定性；P0-03b 同权限可回收 worker、250 ms 硬超时、64 KiB 流级上限、协议/异常退出恢复、父进程退出清理、连续超时退避及合成 500 项延迟/CPU/内存/句柄预算（Conditional Pass；低权限 token、像素 IPC 和真实 provider/支持矩阵未关闭）。
