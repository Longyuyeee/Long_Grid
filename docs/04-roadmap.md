...
2026-08-25 main 资源平台期纠偏：PR #225 以 `9889cdc` 合入后，main run `32804120282` 在原生交互 Surface probe 停止；功能与即时清理均通过，只有 `RepeatedResourcePlateau=false`。审计确认旧探针用未释放的 `Process.GetCurrentProcess()` 观测句柄，观测自身会扰动结果；同时同进程 UIA client proxy 缓存不应冒充产品 provider 泄漏。现改用原生伪句柄/GetProcessHandleCount、一次真实 UIA 与隐藏清理、长生命周期 HWND 三轮 warm-up + 三轮 passive/explicit/passive USER/GDI 平台门，并在销毁前断开 provider。本机 5/5 `Conditional Pass`，Gate A 等待纠正 PR/main CI，详见 [Stage 202](202-main-native-surface-resource-plateau-audit.md)。
