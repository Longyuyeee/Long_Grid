...
- [x] 正式工作区批量选择无障碍人工矩阵入口：BSA-01–BSA-05 分别覆盖纯键盘、Narrator、高对比度、200% 文本缩放和紧凑宽度；启动器要求专用账户与恢复确认，拒绝既有 App 进程，CI 只验证 134-ID/8 控件/播报/断点合同并固定保持 `PendingManualEvidence`。
