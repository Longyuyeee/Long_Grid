...
- [x] Issue #24 SafeMode 空白安全重置：标准无用户状态 v1 配置、锁前/锁内复检、主备损坏证据分别归档、发布失败备份回滚、中断恢复标记与默认取消确认（外部导入、证据清理策略与真实卷结果仍 Pending）。
...
5. 按 Issue #24 复用已建立的正式配置存储；应用关闭排空、单实例、恢复状态 UI、显式备份接受和 SafeMode 空白重置已完成首片验收，外部导入、真实状态入队与真实卷证据继续按独立切片验收；
