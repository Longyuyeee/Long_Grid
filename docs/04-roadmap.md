...
- [x] P0-03a 图标/缩略图异步加载、排队取消与句柄稳定性；P0-03b 受限 Low Integrity 可回收 worker、挂起启动后先加入 Job、显式继承句柄列表、250 ms 硬超时、有界协议/异常退出恢复、父进程 PID/Job Object 双重退出清理、连续超时退避、显式复制匿名映射句柄的有界 BGRA32 IPC、实际 worker 未授权读取暴露、零 Capability AppContainer 启动/令牌/精确 SID ACL/未授权读取阻断/Profile 清理证据及合成 500 项预算（Conditional Pass；真实 worker 迁移、broker 输入方式、正式渲染集成和 provider/支持矩阵未关闭）。
...
- [x] PR #2–#18、#25–#46 已收口，相关代码进入 `main`，旧远端功能分支已删除。
