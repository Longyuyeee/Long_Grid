...
当前功能顺序和验收目标以 [Stage 153 功能对标总文档](153-product-feature-parity-development-plan.md)为准；当前执行队列以[统一开发计划](PRODUCT_EXECUTION_PLAN.md)为唯一来源。PF-020A 的 schema v6 规则、真实预览、原子安全引用分配和真实回归见 [Stage 271](271-pf020a-versioned-rule-preview-safe-assignment-audit.md)，换电脑接续见 [Stage 251](251-current-development-computer-handoff.md)。当前唯一主开发项为 PF-020B；#23/#274、完整 Runtime、签名包、独占可丢弃 Windows 会话和 Stage 216 Guest 准入作为 BOX/M1/TASKBAR/PF-011 真人证据并行门禁，不再冻结安全边界内的产品功能开发。路线图中的勾选表示对应子问题已有代码和报告，不表示完整产品能力已经完成。
...
2026-09-01 Stage 266 PF-010B1 动作广度：删除、布局、文件夹绑定/解绑、引用单个/批量增删、批量改归属、自定义排序和布局恢复进入同一 50 步历史；批量动作保持一步，Failed-save 兼容补偿同步撤回假成功历史。初始布局红测 Actual 为空历史，修正后专项 `20/20`、相关 `33/33`、完整 Release `1,447/1,447`，真实中文文件 SHA-256 不变、Release 0 warning/error、211-ID 合同通过。PF-010B1 工程收口，PF-010B 整体仍 InProgress，唯一接续点为 PF-010B2 重启安全恢复点，详见 [Stage 266](266-pf010b1-unified-history-action-breadth-audit.md)。
...
2026-09-01 Stage 270 PF-011B 正式首次旅程：设置 Store 持久化四态，Customize 进入正式创建，跳过/返回/重跑可恢复，首个真实方格保存成功后完成；隐藏匿名方格、三引用和拖放练习退出。初始红测为旅程属性/方法 `null`；真实 Store 多重启、回滚、旧 v1 和损坏配置 `4/4`，中文旁路文件 SHA-256 不变，最终完整 `1,469/1,469`、Release 0 warning/error、194-ID 合同通过。首轮既有 worker/HWND 时序失败和独立 Pass/Fail/Pass 被保留，未放宽。PF-011 工程收口、真人/物理证据 Pending；唯一接续点为 PF-020A，详见 [Stage 270](270-pf011b-formal-first-run-journey-audit.md)。
...
2026-08-15 M4c2c 专用环境预检：当前机器的仓库、进程、重启信号和磁盘条件通过，但当前交互账户含既有桌面内容，终端未提升，且交流/直流睡眠策略不足 24 小时。复核确认底层 hypervisor 与虚拟计算服务存在，但当前会话没有 Sandbox 入口、Hyper-V 管理命令或 VM 清单权限，也没有已证明可用的专用账户/可抛弃 VM。为避免暴露桌面元数据或产生已知无效证据，本轮未启动 App、worker、采样或计时，也未修改账户、虚拟化和电源设置。M4c 继续 Pending；下一步必须先提供合规专用账户/可抛弃 VM，或明确授权在提升会话中准备可恢复的临时环境，详见 [Stage 149](149-m4c2c-dedicated-environment-preflight-audit.md)。

2026-08-15 G0-01a M4c2c 只读环境门禁：新增独立预检器，使用有限失败码检查五项人工确认、用户/Public Desktop 为空、AC/DC 睡眠关闭、无待重启信号、仓库外空白非重解析证据目录、至少 1 GiB 空间、零产品进程和干净仓库；正式 24 小时入口在任何产品启动前强制调用。入口不创建账户、不改电源/更新/桌面、不启动 VM 或产品，不能产生 M4c Pass。当前主机继续 Rejected，真实 24 小时 Pending，详见 [Stage 151](151-m4c2c-environment-preflight-gate-audit.md)。
...
2026-08-16 PF-002A 桌面空状态入口：权威空工作区现在在主显示器建立有限 DesktopHost 卡片，卡片之外及非按钮区域保持桌面穿透；非注入主点击和标准 UIA Invoke 调度到同一回调，并复用唯一配置提交协调器直接创建默认“新方格”。全量 945/945 和 Release 构建通过。PF-002 仍为 `InProgress`：右键、键盘、就地命名/预览、连续创建和保存失败窗口补偿未完成，详见 [Stage 155](155-pf002-desktop-empty-create-entry-audit.md)。
...
2026-08-25 main 资源平台期纠偏：PR #225 以 `9889cdc` 合入后，main run `32804120282` 在原生交互 Surface probe 停止；功能与即时清理均通过，只有 `RepeatedResourcePlateau=false`。审计确认旧探针用未释放的 `Process.GetCurrentProcess()` 观测句柄，观测自身会扰动结果；同时同进程 UIA client proxy 缓存不应冒充产品 provider 泄漏。现改用原生伪句柄/GetProcessHandleCount、一次真实 UIA 与隐藏清理、长生命周期 HWND 三轮 warm-up + 三轮 passive/explicit/passive USER/GDI 平台门，并在销毁前断开 provider。本机 5/5 `Conditional Pass`；PR #226 两轮最终 CI 通过并以 `3489ca0` 合入，main run `32806544628` 又以 `RepeatedResourcePlateau=true`、`CleanupPassed=true`、1198/1198、90.01%/75.51% 和全部下游门通过。Gate A 已关闭，下一步 PF-006C1，详见 [Stage 202](202-main-native-surface-resource-plateau-audit.md)。

2026-08-25 PF-006C1：正式激活 HWND 新增未修饰 PageUp/PageDown，沿 lifecycle/App 权威视口链复核显式租约、来源、auto-repeat、container/display 和 workspace/topology 代次；新投影在同一锁内 reconcile 后按旧页内偏移恢复新页选择/焦点，同步 lifecycle revision，并只刷新一次 Surface/UIA。真实 HWND SelectionPattern 唯一选中“项目 14”且不抢前台；AutomationId 的匿名序号实际值纠正了测试预期。历史覆盖报告污染曾产生 86.10%/73.41% 假失败，隔离目录完整重跑为 1200/1200、90.36%/75.73%；PR #228 以 `a70a3ef` 合入，PR/main 均为 1200/1200、90.02%/75.58% 和全下游门通过。PF-006C1 工程完成，下一步 PF-006C2，详见 [Stage 203](203-pf006c1-page-navigation-viewport-uia-convergence-audit.md)。
...
2026-08-27 TASKBAR-R2A2b：恢复日志存储下沉到 Core 后，正式 App 启动独立 Worker 取得唯一租约、读取遗留凭据并重新探测 Explorer；无日志、租约竞争、畸形日志、build 变化、未认证准入和缺失原生适配器均有限失败关闭。当前 Windows 10.0.26200.0 的主/副任务栏真实 HWND 均属 Explorer，探测通过但 `DeniedNoCertifiedBuild`；Applied 日志逐字节保留、无 `.new`，前后 HWND/class/PID 不变。新增真实 Worker 专项 4/4、全量 1319/1319、Release 0 warning/error。该阶段不等于任务栏样式或系统默认恢复可用；下一唯一项为 R2A2c 默认不可用原生适配器边界与可丢弃认证矩阵入口，详见 [Stage 214](214-taskbar-r2a2b-startup-recovery-preflight-audit.md)。

2026-08-27 TASKBAR-R2A2c：Core 新增有限原生恢复合同和准入策略，严格复核恢复日志、Windows build、任务栏 class 集合、当前 Explorer 所有权、只读探测结果、认证准入与安全 AdapterId；Explorer PID 变化作为重启事实进入请求而不是复用旧 PID。Worker 目录默认精确为空，正式启动恢复因此仍在任何写调用前返回 `RecoveryDeferredAdapterUnavailable`。受 `LONGGRID_TASKBAR_WORKER_EVIDENCE=1` 保护的真实认证入口由全量测试中的真实 Worker 用例执行，并另有独立 Expected/Actual/Difference 脚本；无开关实际退出 65，Build 26200 实际为 `DeniedNoCertifiedBuild / Unavailable / None`，两次真实探测的 HWND/class/PID 不变且 `ModifiedSystemState=false`。本地 Release 1345/1345、coverage 90.40%/76.00%、build 0 warning/error；PR #262 首轮远端 run `33048393504` 为 1345/1345、90.08%/75.88%，全部后续门通过。该阶段不等于原生清透或恢复已经实现；下一项为仅在可丢弃 Windows 环境执行的 R2B1 `Clear → SystemDefault` 原生适配器实验，详见 [Stage 215](215-taskbar-r2a2c-native-adapter-boundary-audit.md)。

2026-08-27 TASKBAR-R2B1-A：新增可丢弃 Windows Sandbox Host/Guest 双层准入。Host 真实复读 OS/架构/CPU/内存/虚拟化/SLAT/Sandbox 启动器，生成并复读断网、源码只读、证据目录单独可写、关闭剪贴板/音视频/打印重定向且固定 Guest 命令的 `.wsb`；Guest 只证明 `WDAGUtilityAccount` 和两条映射。当前 Host 实际为 Windows 10.0.26200、X64、16 逻辑处理器、42,268,614,656 bytes 内存、虚拟化固件 true，但 SLAT false、Sandbox 启动器缺失，精确返回 `Blocked / mutationAllowed=false / ModifiedSystemState=false`。真实 PowerShell 进程测试 1/1；本地 Release 全量 1346/1346、coverage 90.38%/76.01%、build 0 warning/error；PR #263 首轮远端 run `33050907194` 为 1346/1346、90.09%/75.88%，全部后续门通过。启用 Sandbox 需要提升权限/可能重启，R2B1-B 保持 `EnvironmentBlocked`，不得改在宿主试写，详见 [Stage 216](216-taskbar-r2b1a-disposable-environment-admission-audit.md)。

2026-08-27 TASKBAR-R2B1-A2：在 R2B1-B 环境阻断期间，正式个性化页新增“系统默认 / 通透”两张预设卡片和有限可用性说明；Core 统一复核探测、冲突、Build 认证、原生适配器与恢复事务。当前 App 固定 adapter unavailable，按钮默认禁用且没有任务栏写入事件。Build 26200 两次真实 Worker 探测均为 Pass / DeniedNoCertifiedBuild，主副任务栏 Explorer 身份不变、无冲突、ModifiedSystemState=false、Difference=None；专项 15/15、全量 1353/1353、coverage 90.42%/76.03%、195-ID 合同和 Release build 通过。PR #264 run `33053629094` 同为 1353/1353、90.10%/75.91%，全部下游门成功。可见跨进程 UIA 因已知 WinUI 风险继续 Pending；R2B1-B/R3/R4 未完成，详见 [Stage 217](217-taskbar-r2b1a2-preset-admission-ui-audit.md)。
