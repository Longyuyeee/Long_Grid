...
try {
    if ($ValidateOnly) {
        ...
        Assert-Condition ($LASTEXITCODE -eq 0) `
            'The 129-ID UI source contract validation failed.'
        $uiResult = $uiJson | ConvertFrom-Json
        Assert-Condition (
            $uiResult.contract.requiredAutomationIds -eq 129 -and
            $uiResult.outcome -eq 'Pass'
        ) 'The clean-session chain requires the complete 129-ID UI source contract.'
    ...
    Assert-Condition (
        ...
        $uiResult.outcome -eq 'Pass'
    ) 'The live UIA result did not preserve the 129-ID and clean-session contract.'
